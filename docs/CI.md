...
| `frontend`              | React inputs changed OR full-safe                            | `npm ci`, lint, build, `npm run test:coverage` in `src/Web/ReactApp/`; coverage runs the test type-check ratchet first through the `pretest:coverage` lifecycle hook. |
