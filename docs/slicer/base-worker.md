...
This document describes the hardened base container image for PrintFarmer slicer workers, providing a secure, minimal runtime environment with health monitoring and graceful shutdown capabilities.

## Overview

The base slicer worker container (`Dockerfile.base`) provides:
...
```bash
# Basic run
docker run -p 8080:8080 printfarmer/slicer-worker-base
