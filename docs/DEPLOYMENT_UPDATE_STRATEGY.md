...
Automatic host updates keep their replay anchor and `UpdateAutomation` policy outside the application database and cache. Configure the required absolute `HostUpdates:HostState:RootPath` (or `PFARM__HostUpdates__HostState__RootPath`) to a persistent, writable host volume mounted at the same path after container replacement. The process identity must have read/write/create permission for that directory; the service refuses to start when the path is missing, relative, or unwritable.
