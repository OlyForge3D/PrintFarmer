...
| Fence | `HostUpdateFenceCoordinator` | Proves every registered `IFenceableWriter` (the admission gate; queue-outbox publishing; `PowerReadingPruneService`; `QueueRetentionPruneService`; `BackendStartCommandConsumerService`; `BackendControlCommandConsumerService`; and `BedClearAcknowledgementExpiryService`, each via an independent `IHostUpdateWriterActivityFlag`-derived instance) has actually quiesced before backup — bounded-polled, not assumed. The five interval-driven writers re-check their flags every 250 ms while waiting between passes; the queue-outbox publisher acknowledges only at the top of its next poll, while the API admission writer is gate-shaped rather than loop-shaped. In every shape, acknowledgement follows completion of the preceding iteration's writes; for the interruptible writers the bound is the in-flight iteration plus at most 250 ms, while the publisher's bound includes its poll interval. The executor's availability provider also fails closed (`insufficient_fenced_writers:<names>`) if `HostUpdateExecutionOptions.RequiredFencedWriterNames` names a writer with no registered `IFenceableWriter`. |
