...
`compose_file_missing:...`, `docker_runtime_unavailable`, `insufficient_fenced_writers:webhook-delivery`, or a code-owned `facility_unavailable:...`) so an operator is never left guessing. In the current production code, availability is unconditionally closed by four code-owned blockers: the target-image migration runner is unavailable, protected bootstrap/trusted state for unsigned legacy installs is not established, queue-reconciliation writer fencing is incomplete, and SQL Server visible backup-path mapping is unverified. These are unimplemented facilities, not missing acceptance evidence; they must be implemented before this executor can report production `Available`.
