...
`SystemInfoDto.UpdateScheduling` is additive. It is non-null when the scheduler subsystem is registered, including the current fail-closed unavailable state; `null` is reserved for a subsystem that is not wired. The status reports persisted policy/channel state, due/backoff timestamps, kill-switch state, executor availability, and safe reason codes. The production candidate and executor registrations currently report unavailable because no protected replay anchor, atomic policy mutation facility, or lossless Dallas executor contract exists, and no automatic hosted execution is registered.

The scheduler core (`HostUpdateScheduler` in `src/infra/Services/HostUpdates/HostUpdateScheduler.cs`) implements cadence enforcement, kill-switch checks (at entry and freshly re-read immediately before executor admission), an atomic replay decision (`IHostUpdateReplayStore.DecideAsync`) keyed by enrolled trust root and channel with high-water/sequence/identity/disposition semantics, and a persisted policy fence (`IHostUpdatePolicyFence`) that rejects policy-revision rollback or equal-revision content drift. It remains **fully unwired**: it is not registered in `BackgroundServicesStartup`/`FeatureServicesStartup`, has no hosted service, and is exercised only by its own unit tests. Two integration points are intentionally represented as explicit fail-closed ports rather than functioning production adapters, and must not be treated as available capabilities until real backends exist:

- **Atomic policy mutation.** `IHostUpdatePolicyMutationPort` (`HostUpdatePolicyMutationPort.cs`) is the narrow, expected-revision compare-and-swap contract a real settings-mutation endpoint would need to keep `PolicyRevision` monotonic. `IAppSettingsRepository` has no CAS primitive today, so the only registered implementation, `FailClosedHostUpdatePolicyMutationPort`, always throws `NotSupportedException` and is not wired to any enablement endpoint. The scheduler's file-backed policy fence is a narrower, interim safeguard (revision/fingerprint drift detection only) and is not a substitute for real atomic mutation.
- **Replay anchor.** `IHostUpdateReplayAnchor` detects a syntactically valid but restored/rolled-back replay file by cross-checking a monotonic epoch. No platform-protected anchor facility (e.g. TPM monotonic counter) is implemented; the production default, `UnavailableHostUpdateReplayAnchor`, always fails closed. The replay store therefore cannot be safely deployed until a real anchor implementation is provided — it is fail-closed by construction rather than self-provisioning.

The scheduler hosted loop and replay/policy mutation facilities remain unregistered in dependency injection until these upstream adapters land. The fail-closed candidate, executor, and status adapters may be registered so operators receive explicit unavailable status and no work can start.
