...
`contents.priorRecoverySet` and `contents.recoveryInstructions` are `false`.
`contents.images` and `contents.infrastructure` are both `true` when the bundle
carries the image set described below and both `false` otherwise; a bundle that
claims one without the other, or carries only part of the set, is rejected.
Remaining work under #2658:
