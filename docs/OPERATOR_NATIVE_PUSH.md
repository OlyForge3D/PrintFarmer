...
- Unique `(InstallationId)` — one active owner per physical app installation.
  A successful registration by another user atomically rotates the existing row's
  `UserId` and `RegistrationVersion`.
