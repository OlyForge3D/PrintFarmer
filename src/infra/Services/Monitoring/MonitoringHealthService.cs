    ...
    ILogger<MonitoringHealthService> logger) : IMonitoringHealthService
{
    ...
    public async Task<MonitoringMetricsSummaryDto> GetMetricsSummaryAsync(CancellationToken cancellationToken = default)
    {
        ...
        var p95Latency = QueryPrometheusAsync(client, prometheusUrl,
            "histogram_quantile(0.95, sum(rate(printfarmer_api_call_duration_ms_bucket[5m])) by (le))", cancellationToken);
        var p99Latency = QueryPrometheusAsync(client, prometheusUrl,
            "histogram_quantile(0.99, sum(rate(printfarmer_api_call_duration_ms_bucket[5m])) by (le))", cancellationToken);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        var pending = new List<Task<MetricStreamEvent[]>>
        {
            ...
            QAsync("p95LatencyMs", "histogram_quantile(0.95, sum(rate(printfarmer_api_call_duration_ms_bucket[5m])) by (le))"),
            QAsync("p99LatencyMs", "histogram_quantile(0.99, sum(rate(printfarmer_api_call_duration_ms_bucket[5m])) by (le))"),
