...
#pragma warning restore CA1303
    ...
    public async Task<bool> SendPasswordResetAsync(string email, string resetLink, CancellationToken ct = default)
    {
        ...
        (string? subject, string? plain, string? html) = renderer.Render("PasswordReset", new Dictionary<string, string>
        {
            ["ResetLink"] = resetLink,
    ...
    {
        var renderer = new EmailTemplateRenderer();
        (string? subject, string? plain, string? html) = renderer.Render("EmailConfirmation", new Dictionary<string, string>
        {
