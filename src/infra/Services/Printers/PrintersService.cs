...
#pragma warning restore CS8603
    ...
    public async Task<bool> SendHomeAsync(Guid id, CancellationToken ct)
    {
        ...
        try
        {
            ...
            string moonrakerUrl = BuildMoonrakerUrl(p);
...
#pragma warning restore S2589
    ...
    public async Task<CommandResult> LoadFilamentAsync(Guid id, CancellationToken ct)
    {
        ...
        try
        {
            ...
            string url = BuildMoonrakerUrl(p);
...
#pragma warning restore S6580
    ...
    /// <summary>
    /// Builds the Moonraker API URL using the configured backend port.
    /// The frontend port is reserved for web UI and relative camera URLs.
    /// </summary>
    private static string BuildMoonrakerUrl(Printer printer)
    ...
    public async Task<PrinterDto?> RefreshCameraUrlsAsync(Guid id, CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (_capabilityFactory.TryGetConfiguredCameraDetectionClient(backend, out ISupportsConfiguredCameraDetection? detectionClient) && detectionClient != null)
            {
                ...
                // Camera discovery resolves relative stream URLs against this UI endpoint.
    ...
    private async Task<byte[]?> TryGetSnapmakerU1CameraSnapshotAsync(Printer printer, CancellationToken ct)
    {
        try
        {
            ...
            string moonrakerUrl = BuildMoonrakerUrl(printer);
