...
public sealed class FileHostUpdateAutomationPolicyRepository : IHostUpdateAutomationPolicyRepository, IDisposable
{
    ...
    {
        if (!File.Exists(_path)) return new(true, new HostUpdateAutomationPolicy(), null);
        try
        {
    ...
    public async Task<HostUpdatePolicyReadResult> ReplaceAsync(HostUpdateAutomationPolicy policy, long expectedRevision, CancellationToken ct)
    {
        ...
        if (!IsValid(policy) || expectedRevision < 0) return new(false, new(), "host_update_policy_invalid");
        ...
        try
        {
            ...
            if (current.Policy.Revision != expectedRevision) return new(false, current.Policy, "host_update_policy_revision_conflict");
