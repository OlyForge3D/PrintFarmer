...
public sealed class FileHostUpdateReplayAnchor : IHostUpdateReplayAnchor, IHostUpdateReplayAnchorProvisioner, IDisposable
{
    ...
    public async Task ProvisionAsync(CancellationToken ct)
    {
        ...
        try
        {
            if (File.Exists(_snapshotPath) || File.Exists(_journalPath))
            {
                throw new InvalidOperationException("host_update_replay_anchor_already_provisioned");
            }
    ...
    private List<AnchorEntry> ReadJournal()
    {
        ...
        foreach (string line in File.ReadLines(_journalPath))
        {
            ...
            {
                throw new InvalidDataException("host_update_replay_anchor_truncated", ex);
            }

            if (entry is null || entry.Version != Version || entry.Epoch < 0 || entry.PreviousHash != previous || entry.Hash != Convert.ToHexStringLower(SHA256.HashData(Encoding.UTF8.GetBytes($"{Version}|{entry.Epoch}|{entry.PreviousHash}"))))
            {
