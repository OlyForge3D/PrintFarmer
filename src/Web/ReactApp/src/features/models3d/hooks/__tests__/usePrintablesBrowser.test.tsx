...
describe("usePrintablesBrowser", () => {
  ...
  it("passes @handle username through when requesting public collections", async () => {
    ...
    await waitFor(() => {
      expect(apiClient.getPrintablesUserCollections).toHaveBeenCalledWith(
        "@maker_jane",
        {
          cursor: undefined,
          limit: 8,
  ...
  it("fetches collection models only after expand is enabled", async () => {
    ...
    await waitFor(() => {
      expect(apiClient.getPrintablesCollectionModels).toHaveBeenCalledWith(
        "collection-1",
        {
          cursor: undefined,
          limit: 24,
  ...
  it("uses offset pagination for search and advances using offset + limit", async () => {
    ...
    const { result } = renderHook(() => usePrintablesSearch("tool"), {
      wrapper,
    });

    await waitFor(() => {
      expect(apiClient.searchPrintablesModels).toHaveBeenNthCalledWith(
        1,
        "tool",
        {
          offset: 0,
