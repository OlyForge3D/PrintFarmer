...
import { Button, Checkbox, Input, Spinner } from '@/common/components/ui';
...
export const FileRow: React.FC<FileRowProps> = ({
  ...
  return (
  <div className={`px-4 py-3 flex items-center transition-colors ${selected ? 'bg-pf-bg-2' : 'hover:bg-[var(--pf-hover-overlay)]'}`} onContextMenu={onContextMenu}>
      <Checkbox
        checked={selected}
            ...
            >
              ...
              {hashing && (
                <span className="absolute inset-0 flex items-center justify-center">
                  <Spinner className="w-3 h-3 text-pf-success" />
                </span>
              )}
            </Button>
          )}
