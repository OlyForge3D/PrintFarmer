    ...
    <Card>
      ...
      <Card.Body className="space-y-4">
        ...
        <dl className="grid gap-2 sm:grid-cols-2">
          ...
          <div><dt>Compatibility</dt><dd>{inventory.compatibilityState}: {inventory.compatibilityReasons.join(', ')}</dd></div>
          <div><dt>Normal update eligibility</dt><dd>{inventory.eligibility}: {inventory.eligibilityReasons.join(', ')}</dd></div>
          <div><dt>Inventory collected</dt><dd>{timestamp(inventory.collectedAt)}</dd></div>
        </dl>
        {insider && <Alert variant="warning" title="Insider channel">{INSIDER_WARNING}</Alert>}
        {inventory.compatibilityState === 'MixedChannel' && <Alert variant="error" title="Mixed channels — blocked / unsafe">Stable and insider applications are not a healthy compatible update set.</Alert>}
        <section aria-label="Loaded frontend assets" className="space-y-2">
          ...
          {assetSkew && <Alert variant="warning" title="Frontend/API build mismatch — refresh required">Frontend/API compatibility: Incompatible under the same-build policy. Cached assets differ from this API. Refresh the page to load current assets; if the mismatch remains, reconcile the deployment. Compatibility is not established by refresh alone.</Alert>}
        </section>
        <Table>
          <TableHead><TableRow>
