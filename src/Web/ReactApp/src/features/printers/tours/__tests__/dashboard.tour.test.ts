...
describe('dashboard tour definition', () => {
  it('exports a valid array of tour steps', () => {
    expect(Array.isArray(dashboardTour)).toBe(true);
    expect(dashboardTour.length).toBeGreaterThan(0);
  });

  it('has a reasonable number of steps (4-8)', () => {
    expect(dashboardTour.length).toBeGreaterThanOrEqual(4);
    expect(dashboardTour.length).toBeLessThanOrEqual(8);
  });

  it('each step has an element selector', () => {
    for (const step of dashboardTour) {
  ...
  it('steps reference data-tour-* selectors for stability', () => {
    for (const step of dashboardTour) {
      // Steps should use data-tour attributes for refactor-proof targeting
