...
import { act, fireEvent, render as renderTree, screen, within, waitFor } from '@testing-library/react';
...
vi.mock('@/services/api', () => ({ apiClient: {
  put: async (_url: string, body: unknown) => ({ data: { userId: 'motion-user', rowVersion: 'v2', ...(body as object) } }),
} }));
vi.mock('sonner', () => ({ toast: { success: vi.fn(), error: (...args: unknown[]) => mockToastError(...args), info: vi.fn() } }));
...
const capabilities = {
  supportsMovement: true, supportsControlOperations: true, supportsTemperatureControl: true,
  supportsFilamentControl: false, supportsObjectExclusion: false,
...
function Controls({ surface }: { surface: Surface }) {
  return surface === 'detail'
    ...
    : <PrinterDetailsSidebar printerId={printer.id} printer={printer} backendCapabilities={capabilities} onClose={vi.fn()} />;
}
function fillCoordinates(scope: ReturnType<typeof within> = screen) {
  for (const [axis, value] of [['X', '120'], ['Y', '130'], ['Z', '15']]) {
    fireEvent.change(scope.getByLabelText(`${axis} absolute target`), { target: { value } });
  }
}
...
function render(element: ReactElement) {
  ...
  client.setQueryData(USER_SETTINGS_KEY, { userId: 'motion-user', printerControlMode: accountMode, rowVersion: 'v1' });
...
describe.each<Surface>(['detail', 'sidebar'])('%s initiating motion controls', surface => {
  it.each([
  ...
  ])('shows activity only on %s immediately, preserving the exact motion intent', async (name, intent) => {
    let finish!: () => void;
    mockExecute.mockImplementation(() => new Promise(resolve => { finish = () => resolve({ success: true }); }));
    ...
    expect(container.querySelectorAll('button[aria-busy="true"]')).toHaveLength(1);
    expect(screen.getByRole('button', { name: 'Home all axes' })).toBeDisabled();
    expect(screen.getByRole('button', { name: 'Jog Y positive' })).toBeDisabled();
  ...
  it('does not mark any initiating button busy during initial availability checking', () => {
    ...
    const { container } = render(<Controls surface={surface} />);
    expect(screen.getByRole('button', { name: 'Home all axes' })).toBeDisabled();
    expect(container.querySelectorAll('button[aria-busy="true"]')).toHaveLength(0);
  ...
  it('clears activity and retains actionable failure feedback in Expert', async () => {
    accountMode = 'Expert';
    let finish!: () => void;
    mockExecute.mockImplementation(() => new Promise(resolve => { finish = () => resolve({ success: false, error: 'Printer rejected motion' }); }));
...
it('synchronizes actual detail/sidebar modes and keeps both coordinate rows', async () => {
  render(<><section aria-label="detail"><Controls surface="detail" /></section><section aria-label="sidebar"><Controls surface="sidebar" /></section></>);
  const detail = within(screen.getByRole('region', { name: 'detail' }));
  const sidebar = within(screen.getByRole('region', { name: 'sidebar' }));
  expect(detail.getByRole('button', { name: 'Guided' })).toHaveAttribute('aria-pressed', 'true');
  expect(sidebar.getByRole('button', { name: 'Guided' })).toHaveAttribute('aria-pressed', 'true');
  fireEvent.click(detail.getByRole('button', { name: 'Expert' }));
  await waitFor(() => expect(sidebar.getByRole('button', { name: 'Expert' })).toHaveAttribute('aria-pressed', 'true'));
