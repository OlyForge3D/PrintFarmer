...
export function PartsInventoryPage() {
  ...
  // include the server-composed `printedPartsInventoryEnabled` operator
  // feature flag (#714). This tells us up front — with no
  // parts-inventory-specific network call at all — whether the feature is
  // disabled, instead of firing the reorder endpoint and inspecting it for
  // a `featureDisabled` 404 (the prior #1686 fix, which fixed the SKUs/Bins
  // tab queries but left the reorder probe itself as an always-firing
  // request against a disabled route — issue #1761). Older API builds omit
  // `operatorFeatures` entirely; per its documented contract the frontend
  // must default an absent flag to enabled, so only a literal `false`
  // means disabled.
  const { data: capabilities, status: capabilitiesStatus } = useSystemCapabilities();
  const capabilitiesSettled = capabilitiesStatus !== 'pending';
  const featureDisabled = capabilities?.operatorFeatures?.printedPartsInventoryEnabled === false;

  // Until capabilities have settled we don't yet know whether the feature is
  // enabled. Rendering the tabs (or firing the reorder query) before then
  // would race a possibly-disabled feature. Because the capabilities query
  // is cached app-wide and typically already resolved by the time this page
  // mounts, this is normally instantaneous.
  const featureStatusKnown = capabilitiesSettled;

  // Only fire the reorder query — used for the page-level badge count —
  // once we know the feature is enabled. When disabled, this query never
  // runs, so the disabled page never issues the parts-inventory reorder
  // request the tabs below are gated on anyway.
  const { data: reorderCandidates = [] } = useReorderCandidates({
    enabled: featureStatusKnown && !featureDisabled,
  });
