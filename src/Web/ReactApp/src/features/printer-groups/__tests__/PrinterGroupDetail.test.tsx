...
import {
  PrinterBackend,
  type PrinterGroupDetail as PrinterGroupDetailType,
} from "@/types/api";
...
vi.mock("../components/PrinterAssignment", () => ({
  PrinterAssignment: ({
    ...
    groupId: string;
    assignedPrinters: unknown[];
  }) => (
    <div data-testid="printer-assignment" data-group-id={groupId}>
      Assignment Section
    </div>
  ),
}));
...
vi.mock("../components/AccessControlTab", () => ({
  AccessControlTab: ({ groupId }: { groupId: string }) => (
    <div data-testid="access-control-tab" data-group-id={groupId}>
      Access Control
    </div>
  ),
}));
...
describe("PrinterGroupDetail", () => {
  ...
  it("shows error state when fetch fails", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Failed to load group details/),
      ).toBeInTheDocument();
    });
  });
  ...
  it("renders PrinterAssignment component", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("printer-assignment")).toHaveAttribute(
        "data-group-id",
        "g1",
      );
    });
  ...
  it("calls onEdit when edit button is clicked", async () => {
    ...
    expect(onEdit).toHaveBeenCalledWith(
      expect.objectContaining({ id: "g1", name: "Test Group" }),
    );
  });
