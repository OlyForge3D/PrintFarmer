...
const { mockToast, mockAssignPrinterToGroup, mockRemovePrinterFromGroup } =
  vi.hoisted(() => ({
...
vi.mock("@/services/api", () => ({
  apiClient: {
    assignPrinterToGroup: (...args: unknown[]) =>
      mockAssignPrinterToGroup(...args),
    removePrinterFromGroup: (...args: unknown[]) =>
      mockRemovePrinterFromGroup(...args),
  },
}));
...
describe("PrinterAssignment", () => {
  ...
  it("shows empty state when no printers are assigned", () => {
    ...
    expect(
      screen.getByText("No printers assigned to this group"),
    ).toBeInTheDocument();
  });
