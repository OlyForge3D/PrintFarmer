...
const makePrinter = (
  overrides: Partial<LocationSubtreePrinter> = {},
): LocationSubtreePrinter => ({
  printerId: "p1",
...
describe("computeStats", () => {
  ...
  it("counts active jobs with the same predicate as the page list", () => {
    const printers = [
      makePrinter({
        printerId: "printing-with-job",
        status: "Printing",
        currentJobName: "gearbox.gcode",
      }),
      makePrinter({
        printerId: "printing-without-job",
        status: "Printing",
        currentJobName: null,
      }),
      makePrinter({
        printerId: "paused-with-job",
        status: "Paused",
        currentJobName: "paused.gcode",
      }),
      makePrinter({
        printerId: "idle-with-whitespace-job",
        status: "Idle",
        currentJobName: "   ",
      }),
    ];
