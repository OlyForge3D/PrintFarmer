...
type HashData = { fileName: string; size: number; algorithm: string; hash: string };
type CacheEntry = { d: HashData; t: number };
...
export function useFileHash(path: string | undefined, algorithm: 'sha256' | 'sha1' = 'sha256') {
  ...
  let initialData: HashData | undefined;
  ...
  if (enabled) {
    try {
      ...
      if (raw) {
        ...
        const entry = map[k];
        if (entry && entry.d && typeof entry.d.hash === 'string') {
          initialData = entry.d;
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (entries.length > 500) {
          ...
          const newMap: Record<string, CacheEntry> = {};
