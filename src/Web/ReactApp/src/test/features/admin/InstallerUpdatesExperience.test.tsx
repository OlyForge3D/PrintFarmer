...
describe('InstallerUpdatesExperience', () => {
  it('keeps execution inaccessible to view-only users and explains the security prerequisites', async () => {
    ...
    render(<InstallerUpdatesExperience inventory={inventory()} refetch={vi.fn()} />);
  ...
  it('labels service identities as observed/installed and keeps the target identity unknown', async () => {
    render(<InstallerUpdatesExperience inventory={inventory({ selectedChannel: 'insider', observedChannel: 'stable', targetChannel: 'stable', eligibility: 'Blocked', compatibilityState: 'MixedChannel', services: [replica({ identity })] })} refetch={vi.fn()} />);
  ...
  it('announces offline observation in a live status region and reconciles after reconnect', async () => {
    const refetch = vi.fn();
    render(<InstallerUpdatesExperience inventory={inventory()} refetch={refetch} />);
  ...
  it('presents blocked readiness even when eligibility is NotManaged and compatibility is Compatible', () => {
    render(<InstallerUpdatesExperience inventory={blockedReadinessInventory()} refetch={vi.fn()} />);
  ...
  it('renders every observed replica and marks conflicting identities without proposing a target', async () => {
    render(<InstallerUpdatesExperience inventory={conflictingReplicaInventory()} refetch={vi.fn()} />);
    expect(screen.getByText(/Conflicting observed identities/)).toBeVisible();
