...
describe("FailureDetectionMonitoringBadge", () => {
  ...
  it("exposesStateInTooltip_BeforePrintBegins_ShowsCheckingState", () => {
    ...
    expect(button).toHaveAttribute(
      "title",
      expect.stringContaining("click for details"),
    );
  });
  ...
  it("exposesStateInTooltip_MonitoringState_ShowsGuarding", () => {
    ...
    const button = screen.getByRole("button", {
      name: /open spaghetti detection details for prusa mk4/i,
    });

    expect(button).toHaveAttribute(
      "title",
      expect.stringContaining("Guarding"),
    );
    expect(button).toHaveAttribute(
      "title",
      expect.stringContaining("click for details"),
  ...
  it("opensModal_WhenClicked_ShowsDetailedContext", () => {
    ...
    expect(
      screen.getAllByText("Failed to contact Obico ML service."),
    ).toHaveLength(2);
    expect(
      screen.getByText(
    ...
    expect(
      screen.getByText("Failure detected (92% confidence)"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Triggered on the last result"),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getAllByText("Failure incident detected").length,
    ).toBeGreaterThan(0);
    ...
    ).toHaveAttribute("href", "http://example.com/failure.jpg");
    expect(
      screen
        .getAllByRole("link", { name: /open incident snapshot/i })
        .some(
          (link) =>
            link.getAttribute("href") === "http://example.com/history.jpg",
        ),
    ).toBe(true);
  });
  ...
  it("remainsClickable_OpensModal", () => {
    ...
    const button = screen.getByRole("button", {
      name: /open spaghetti detection details/i,
    });
  ...
  it("liveIsPrinting_OverridesStaleDto_WhenPrinterIsActivePrinting", () => {
    ...
    const button = screen.getByRole("button", {
      name: /open spaghetti detection details for bambu x1/i,
    });
    ...
    expect(
      screen.queryByText(/printer is not printing/i),
