...
const mockComputeMaterialCostPerGram = vi.fn(
  (grams: number | null | undefined, costPerGram: number | null | undefined): number | null => {
    if (grams == null || costPerGram == null) {
      return null;
    }
...
const mockComputeMaterialCost = vi.fn(
  (grams: number | null | undefined, costPerKg: number | null | undefined): number | null => {
    if (grams == null || costPerKg == null) {
      return null;
    }
    return (grams / 1000) * costPerKg;
  },
);
const mockFormatPrintTime = vi.fn((seconds: number) => {
  void seconds;
  return '1h 0m';
});
const mockFormatFilamentUsed = vi.fn((grams: number) => {
  void grams;
  return '50.0g';
});
...
vi.mock('@/services/sliceJobService', () => ({
  sliceJobService: {
    computeMaterialCostPerGram: (grams: number, costPerGram: number) => mockComputeMaterialCostPerGram(grams, costPerGram),
    computeMaterialCost: (grams: number, costPerKg: number) => mockComputeMaterialCost(grams, costPerKg),
