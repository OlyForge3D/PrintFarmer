...
type DeleteConfirmationModalProps = PrinterDeleteProps | GenericDeleteProps;

export function DeleteConfirmationModal(props: DeleteConfirmationModalProps) {
  const { isOpen, onConfirm } = props;

  if ('printers' in props && props.printers) {
    const { printers, onCancel } = props;
  ...
  const { onClose, title, message, confirmText = 'Delete', isDeleting = false } = props as GenericDeleteProps;
  ...
  return (
    ...
    >
      <p className="text-pf-text-secondary">{message}</p>
    </Modal>
  );
}
