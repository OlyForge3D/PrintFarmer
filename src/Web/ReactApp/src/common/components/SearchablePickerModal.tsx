...
interface SearchablePickerModalProps<T> {
  ...
  isLoading?: boolean;
  className?: string;
  /** When provided, shows a "Enter URL" tab. Callback receives (url, fileName). */
  onUrlSubmit?: (url: string, fileName: string) => void;
}
...
function SearchablePickerContent<T>({
  ...
  const urlFooter = (
    <>
      ...
      <Button variant="primary" disabled={!urlInput.trim()} onClick={handleUrlConfirm}>
        Use URL
      </Button>
    </>
  );
  ...
  return (
    <>
      {/* Tabs (only when URL mode is available) */}
      {onUrlSubmit && (
        <nav className="flex gap-1 mb-4 border-b border-pf-border" aria-label="Model source">
          <Button
            variant="unstyled"
            className={clsx(
              'px-4 py-2 text-sm font-medium border-b-2 transition-colors',
              activeTab === 'library'
                ? 'border-pf-accent-2 text-pf-text-primary'
      ...
      {activeTab === 'library' ? (
        ...
        <>
          ...
          <div className="space-y-4 py-4">
            <div>
              <label htmlFor="url-input" className="block text-sm font-medium text-pf-text-primary mb-1">File URL</label>
              <Input
                id="url-input"
                type="url"
                value={urlInput}
            ...
            <div>
              <label htmlFor="filename-input" className="block text-sm font-medium text-pf-text-primary mb-1">
                File Name <span className="text-pf-text-muted font-normal">(optional — derived from URL)</span>
              </label>
              <Input
                id="filename-input"
                type="text"
