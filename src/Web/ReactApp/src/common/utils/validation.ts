...
/**
 * Hostname prefixes generated by backends that are syntactically valid
 * http(s) URLs (so `isSafeHttpUrl()` accepts them) but are internal-only
 * container/service names that are never resolvable from a browser running
 * on the user's machine. `TestEmulatorSeeder` (src/backends/Farm.Backend.Plugin.TestEmulator)
 * always creates simulated printers with `ServerUrl = http://testemulator-<guid>`,
 * which produces an unresolvable-hostname network error if opened directly
 * (#1546). Extend this list if another backend starts generating similarly
 * unreachable synthetic hostnames.
 */
const INTERNAL_ONLY_HOSTNAME_PREFIXES = ['testemulator-'];
 ...
 * hostnames (see `INTERNAL_ONLY_HOSTNAME_PREFIXES`) in addition to the
...
export function isBrowserReachableUrl(value: string): boolean {
  ...
  try {
    ...
    return !INTERNAL_ONLY_HOSTNAME_PREFIXES.some(prefix => hostname.startsWith(prefix));
  } catch {
    return false;
  }
}
