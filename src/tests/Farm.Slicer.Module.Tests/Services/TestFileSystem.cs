...
namespace Farm.Slicer.Module.Tests.Services
{
    ...
    public class TestFileSystem : IFileSystem
    {
        ...
        public bool DirectoryExists(string path) => _dirs.ContainsKey(NormalizePath(path)) || Directory.Exists(path);

        public void CreateDirectory(string path)
        {
            _dirs[NormalizePath(path)] = true;
        }

        public bool FileExists(string path) => _files.ContainsKey(NormalizePath(path)) || File.Exists(path);

        public void DeleteFile(string path)
        {
            _ = _files.TryRemove(NormalizePath(path), out _);
        }
        ...
        public void MoveFile(string sourceFileName, string destFileName, bool overwrite = false)
        {
            ...
            if (_files.TryRemove(normSrc, out byte[]? data))
            {
                if (!overwrite && _files.ContainsKey(normDest))
                {
                    throw new IOException($"File already exists: {destFileName}");
                }
                _files[normDest] = data;

                // Move timestamps if they exist
                if (_times.TryRemove(normSrc, out (DateTime Creation, DateTime LastWrite) times))
                {
                    _times[normDest] = times;
                }
            }
        ...
        {
            return new TestFileStream(this, NormalizePath(path));
        }

        public Stream OpenRead(string path)
        {
            if (_files.TryGetValue(NormalizePath(path), out byte[]? data))
            {
        ...
        {
            return _files.TryGetValue(NormalizePath(path), out byte[]? data)
        ...
        {
            return _files.TryGetValue(NormalizePath(path), out byte[]? data) ? Encoding.UTF8.GetString(data) : File.ReadAllText(path);
        }

        public string[] GetFiles(string path, string searchPattern, SearchOption option)
        {
            string prefix = NormalizePath(path).TrimEnd(Path.DirectorySeparatorChar) + Path.DirectorySeparatorChar;
        ...
        public FileInfoData GetFileInfo(string path)
        {
            ...
            if (_files.TryGetValue(norm, out byte[]? data))
            {
                (DateTime Creation, DateTime LastWrite) times = _times.GetOrAdd(norm, _ => (DateTime.UtcNow, DateTime.UtcNow));
        ...
        internal void Commit(string path, byte[] data)
        {
            string norm = NormalizePath(path);
            _files[norm] = data;
            _times[norm] = (DateTime.UtcNow, DateTime.UtcNow);
        }

        internal void SetCreationTimeUtc(string path, DateTime when)
        {
            string norm = NormalizePath(path);
            _ = _times.AddOrUpdate(norm, _ => (when, when), (k, v) => (when, v.LastWrite));
        }

        internal void SetLastWriteTimeUtc(string path, DateTime when)
        {
            string norm = NormalizePath(path);
            _ = _times.AddOrUpdate(norm, _ => (when, when), (k, v) => (v.Creation, when));
        }
