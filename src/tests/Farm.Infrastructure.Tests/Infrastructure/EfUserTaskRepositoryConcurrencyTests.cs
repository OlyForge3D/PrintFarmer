...
public class EfUserTaskRepositoryConcurrencyTests
{
    ...
    public async Task TrackUpdateAsync_ConcurrentUserStatusChange_IsNotClobbered()
    {
        ...
        await using AppDbContext compilerCtx = AppDbTestHelpers.CreateContext(connection);
        ...
        await using (AppDbContext userCtx = AppDbTestHelpers.CreateContext(connection))
        {
        ...
        await using AppDbContext verifyCtx = AppDbTestHelpers.CreateContext(connection);
    ...
    {
        using SqliteConnection connection = AppDbTestHelpers.CreateOpenSqliteConnection();
        await using AppDbContext ctx = AppDbTestHelpers.CreateContext(connection, ensureCreated: true);
    ...
    public async Task TryUpdateFieldsIfOpenAsync_ProfileImportRacesUserSkip_TerminalRowUnchanged()
    {
        ...
        UserTask detached;
        await using (AppDbContext readCtx = AppDbTestHelpers.CreateContext(connection))
        {
        ...
        await using (AppDbContext importCtx = AppDbTestHelpers.CreateContext(connection))
        {
    ...
    public async Task TryUpdateFieldsIfOpenAsync_ProfileImportExpectedUpdatedAt_RejectsStaleSnapshot()
    {
        ...
        await using AppDbContext secondContext = AppDbTestHelpers.CreateContext(connection);
        ...
        await using AppDbContext verifyContext = AppDbTestHelpers.CreateContext(connection);
    ...
    public async Task CompileAsync_Bootstrap_ActiveSuppressedSourceOlderThanMaximumAge_RecreatesTask()
    {
        ...
        await using AppDbContext ctx = AppDbTestHelpers.CreateContext(connection);
        EfUserTaskRepository repo = new(ctx);
