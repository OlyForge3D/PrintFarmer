...
public class PrusaSlicerDockerIntegrationTests : IAsyncLifetime
{
    ...
    public async Task PrusaSlicerWorker_ShouldBuildDockerImage_Successfully()
    {
        ...
        (bool Success, string Output, string ErrorOutput) result = await DockerTestHelpers.RunDockerCommandAsync(_output, _baseDirectory, "build", "-f", "Dockerfile.prusaslicer", "-t", "prusaslicer-worker-test", ".");
