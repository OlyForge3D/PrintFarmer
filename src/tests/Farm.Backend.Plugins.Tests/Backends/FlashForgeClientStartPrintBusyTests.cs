...
public sealed class FlashForgeClientStartPrintBusyTests
{
    ...
    /// <summary>
    /// Starts a TCP server on an OS-assigned loopback port that accepts one connection per
    ...
    {
        // Bind port 0 directly so the listener owns the OS-assigned port (#3025).
