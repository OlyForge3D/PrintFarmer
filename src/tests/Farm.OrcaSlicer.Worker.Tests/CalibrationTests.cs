...
public class CalibrationTests : IDisposable
{
    ...
    #region CalibrationParameters — Unrelated Key Tolerance

    [Fact]
    public void CalibrationParameters_Parse_TemperatureTowerWithExtraneousUnrelatedStringKey_NumericFieldsStillParseCorrectly()
    {
        // Regression coverage for the per-key JsonDocument-based Parse rewrite: a payload
        // combining an unrelated method's numeric field with a stray string key must not regress
        // the numeric method's own parsing. Under a naive whole-payload
        // Dictionary<string,double> deserialization this JSON would have thrown on the string
        // value and silently fallen back to *all* TemperatureTower defaults, hiding the
        // client-supplied start_temperature entirely.
        CalibrationParameters parameters = CalibrationParameters.Parse(
            """{"start_temperature": 220, "notes_ref": "marlin"}""",
