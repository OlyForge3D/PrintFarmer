...
public sealed class PrinterCalibrationControllerTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task CalibrationRoutes_InSplitDeployment_ReturnProfileServiceUnavailable()
    {
        await using SlicerDisabledWebApplicationFactory splitFactory = new(
            "split",
            new Dictionary<string, string?>
            {
                ["Testing:UseTestAuthentication"] = "true",
        ...
        using HttpClient client = splitFactory.CreateClient();
