...
public class MoonrakerClientTests
{
    ...
    [Fact]
    public async Task GetStatusAsync_ReturnsOnlineAndState()
    {
    ...
    [Fact]
    public async Task GetPrinterInfoAsync_ParsesWrappedInfo()
    {
        var (client, _, _) = CreateClient(req =>
        {
            req.RequestUri!.AbsolutePath.Should().Be("/printer/info");
    ...
    [Fact]
    public async Task GetJobAsync_WhenPrinting_ReturnsProgressJobNameAndThumb()
    {
        var (client, _, _) = CreateClient(req =>
        {
    ...
    [Fact]
    public async Task GetCompositeStatusAsync_AggregatesStateJobPositionTempsAndCamera()
    {
    ...
    [Fact]
    public async Task GetCameraSnapshotAsync_FetchesSnapshotBytes()
    {
        var (client, _, _) = CreateClient(req =>
        {
            var url = req.RequestUri!.ToString();
    ...
    public async Task GcodeCommandEndpoints_SendExpectedScript(string expectedScript, string methodName)
    {
        string? body = null;
        var (client, _, _) = CreateClient(req =>
        {
        ...
        mi.Should().NotBeNull();
        var task = (Task<bool>)mi!.Invoke(client, new object?[] { Base, CancellationToken.None })!;
        var ok = await task;
        ok.Should().BeTrue();
    ...
    [Fact]
    public async Task SetTempsAsync_PostsM104AndM140()
    {
        string? body = null;
        var (client, _, _) = CreateClient(req =>
        {
    ...
    [Fact]
    public async Task MoveAsync_UsesRelativeModeAndResetsAbsolute()
    {
        string? body = null;
        var (client, _, _) = CreateClient(req =>
        {
    ...
    [Fact]
    public async Task MoveToAsync_UsesAbsoluteMove()
    {
        string? body = null;
        var (client, _, _) = CreateClient(req =>
        {
    ...
    [Fact]
    public async Task StartPrintAsync_PostsFilename()
    {
        string? body = null;
        var (client, _, _) = CreateClient(req =>
        {
    ...
    [Fact]
    public async Task GetFileListAsync_ReturnsGcodeNames()
    {
        var (client, _, _) = CreateClient(req =>
        {
        ...
        list.Should().BeEquivalentTo(new[] { "a.gcode", "sub/c.gcode" });
    }

    [Fact]
    public async Task GetFileRootsAsync_ReturnsRoots()
    {
        var (client, _, _) = CreateClient(req =>
        {
    ...
    [Fact]
    public async Task GetDirectoryAsync_ReturnsDirectoryInfo()
    {
        var (client, _, _) = CreateClient(req =>
        {
    ...
    [Fact]
    public async Task CreateDirectoryAsync_PostsAndParsesResponse()
    {
        string? body = null;
        var (client, _, _) = CreateClient(req =>
        {
    ...
    [Fact]
    public async Task DeleteFileOrDirectoryAsync_CallsDeleteWithForceFlag()
    {
        var (client, _, _) = CreateClient(req =>
        {
    ...
    [Fact]
    public async Task MoveAndCopyFileAsync_PostsSourceAndDest()
    {
    ...
    [Fact]
    public async Task GetFileMetadata_StartMetadataScan_AndGetThumbnail()
    {
        var (client, _, _) = CreateClient(req =>
        {
            ...
            {
                return new HttpResponseMessage(HttpStatusCode.OK) { Content = new ByteArrayContent(new byte[] { 9, 8 }) };
            }
            return new HttpResponseMessage(HttpStatusCode.NotFound);
        });
    ...
    [Fact]
    public async Task DownloadAndDeleteAndStreamFile_Work()
    {
        var (client, _, _) = CreateClient(req =>
        {
    ...
    [Fact]
    public async Task GetDetailedFileListAsync_ReturnsExtendedEntries()
    {
        var (client, _, _) = CreateClient(req =>
        {
            req.RequestUri!.AbsolutePath.Should().Be("/server/files/list");
    ...
    [Fact]
    public async Task HistoryEndpoints_Work()
    {
        var (client, _, _) = CreateClient(req =>
        {
            var url = req.RequestUri!.ToString();
    ...
    [Fact]
    public async Task SpoolmanEndpoints_Work()
    {
        var (client, _, _) = CreateClient(req =>
        {
            var url = req.RequestUri!.ToString();
