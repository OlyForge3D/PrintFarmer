...
public class PasswordPolicyServiceTests
{
    ...
    public async Task UpdateAsync_CreatesOrUpdates_AndReturnsDto()
    {
        ...
        PasswordPolicyService svc = new PasswordPolicyService(repo.Object);
        UpdatePasswordPolicyRequest request = new UpdatePasswordPolicyRequest { MinLength = 14, RequireDigit = true };

        PasswordPolicyDto result = await svc.UpdateAsync(request, CancellationToken.None);
