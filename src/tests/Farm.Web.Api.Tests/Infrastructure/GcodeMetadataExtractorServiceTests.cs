...
";
    ...
    [Fact]
    public async Task ExtractPrinterModel_SkipsTemplatePlaceholderWithTrailingEscapes()
    {
        // OrcaSlicer config blocks may have [printer_model]\n as a template placeholder
        // The parser must skip these and find the real value
        string gcodeContent =
            "; generated by OrcaSlicer 2.2.0 on 2026-03-09\n" +
            "; filament_type = PLA\n" +
            "; nozzle_diameter = 0.4\n" +
            "; machine_start_gcode = ;printer_model:[printer_model]\\n;initial_filament:PLA\n" +
    ...
    [Fact]
    public async Task ExtractPrinterModel_SkipsTemplatePlaceholderWithCurlyBraces()
    {
        string gcodeContent =
        ...
        result.PrinterModel.Should().BeNull();
    }

    [Fact]
    public async Task ExtractPrinterModel_MatchesMachineName()
    {
        // Cura uses machine_name instead of printer_model
        string gcodeContent =
            "; Cura_Version:5.0.0\n" +
            "; machine_name = Prusa CORE One\n" +
            "G28\n";

        GcodeMetadataExtracted result = await _service.ExtractMetadataAsync(gcodeContent);

        result.PrinterModel.Should().Be("Prusa CORE One");
    }

    [Fact]
    public async Task ExtractPrinterModel_PrefersFirstMatchOverConfigBlock()
    {
        // OrcaSlicer start gcode has resolved value at top, template in config block
        string gcodeContent =
            ";printer_model:Elegoo Centauri Carbon\n" +
            ";initial_filament:PLA\n" +
            "G28\n" +
