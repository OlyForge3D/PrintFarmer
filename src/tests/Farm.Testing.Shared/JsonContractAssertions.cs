...
/// <summary>
/// Assertion helpers for wire-contract corpus tests (issue #2238). Every assertion here
/// operates on <see cref="JsonElement"/> (parsed from real serialized bytes) — never on a
/// deserialized CLR object — because a CLR-object assertion cannot detect a serializer
/// misconfiguration (camelCase vs snake_case, string enum vs numeric enum, unexpected
/// null-handling, etc.); it can only detect that the deserializer papered over the wire shape.
/// </summary>
public static class JsonContractAssertions
{
    ...
    /// list means the trees are equivalent. Used both to guard the checked-in corpus against
    /// silent drift and by the negative-control meta-tests that prove a camelCase→snake_case
    /// rename or a string-enum→numeric-enum swap is actually caught.
    /// </summary>
...
/// <summary>Thrown when a wire-contract corpus assertion fails.</summary>
