...
public class DatabaseInitializer(AppDbContext context, ILogger<DatabaseInitializer> logger, IDataSeedService dataSeedService) : IDatabaseInitializer
{
    ...
    private async Task SeedResourcesAsync()
    {
        var resources = new[]
        {
            ...
            new { Name = "calibration", DisplayName = "Printer Calibration", ResourceType = "calibration", Description = "Printer calibration projects" },
            ...
            new { Name = "slicing", DisplayName = "Slicing Submission & Artifacts", ResourceType = "slicer", Description = "Farm-user-reachable slicing actions: submit/read-artifact/promote (SlicePrintBridgeController, GcodePromotionsController). Distinct from \"slicer_engines\", which gates farm_admin-only slicer worker/engine management." },
