...
public interface ICalibrationProjectService
{
    ...
    Task<IReadOnlyList<GeneratedProfileRevisionDto>> GetGeneratedProfilesAsync(
        Guid projectId,
        CalibrationActor actor,
        CancellationToken cancellationToken);

    Task<CalibrationApiResult<GeneratedProfileRevisionDto>> GetGeneratedProfileAsync(
        Guid revisionId,
        CalibrationActor actor,
        CancellationToken cancellationToken);

    Task<CalibrationApiResult<GeneratedProfileRevisionDto>> CreateGeneratedProfileAsync(
        Guid projectId,
        GeneratedProfileRevisionCreateRequest request,
        CalibrationActor actor,
        CancellationToken cancellationToken);

    Task<CalibrationApiResult<GeneratedProfileRevisionDto>> RecordGeneratedProfileOperationAsync(
        Guid revisionId,
        GeneratedProfileRevisionOperationRequest request,
    ...
    : ICalibrationProjectService
{
    ...
    /// <inheritdoc />
    public async Task<IReadOnlyList<GeneratedProfileRevisionDto>> GetGeneratedProfilesAsync(
        Guid projectId,
        CalibrationActor actor,
        CancellationToken cancellationToken)
    {
        if (await FindVisibleProjectAsync(projectId, actor, false, cancellationToken) is null)
        {
            return [];
        }

        return (await _dbContext.GeneratedProfileRevisions
            .AsNoTracking()
            .Where(revision => revision.ProjectId == projectId)
            .OrderBy(revision => revision.RevisionNumber)
            .ToListAsync(cancellationToken))
            .Select(MapGeneratedProfile)
            .ToArray();
    }

    /// <inheritdoc />
    public async Task<CalibrationApiResult<GeneratedProfileRevisionDto>> GetGeneratedProfileAsync(
        Guid revisionId,
        CalibrationActor actor,
        CancellationToken cancellationToken)
    {
        GeneratedProfileRevision? revision = await FindVisibleGeneratedProfileAsync(
            revisionId,
            actor,
            cancellationToken);
        return revision is null
            ? NotFound<GeneratedProfileRevisionDto>()
            : CalibrationApiResult<GeneratedProfileRevisionDto>.Success(MapGeneratedProfile(revision));
    }

    /// <inheritdoc />
    public async Task<CalibrationApiResult<GeneratedProfileRevisionDto>> CreateGeneratedProfileAsync(
        Guid projectId,
        GeneratedProfileRevisionCreateRequest request,
        CalibrationActor actor,
        CancellationToken cancellationToken)
    {
        ...
        if (string.IsNullOrWhiteSpace(request.ClientId) ||
            string.IsNullOrWhiteSpace(request.GenerationRequestId) ||
            string.IsNullOrWhiteSpace(request.ProfileType) ||
            string.IsNullOrWhiteSpace(request.SchemaVersion) ||
            string.IsNullOrWhiteSpace(request.Name) ||
            string.IsNullOrWhiteSpace(request.SourceProfileFingerprint) ||
            string.IsNullOrWhiteSpace(request.GeneratorVersion) ||
            request.Name.Length > 256 ||
            !IsJsonContainer(request.NormalizedSettings) ||
            !string.Equals(request.SlicerEngine, CalibrationContractConstants.SlicerEngine, StringComparison.Ordinal) ||
            !string.Equals(request.SlicerDistribution, CalibrationContractConstants.SlicerDistribution, StringComparison.Ordinal))
        {
            return Validation<GeneratedProfileRevisionDto>("generated_profile_invalid");
        }

        if (ValidateSafeJson(request.NormalizedSettings) is string normalizedSettingsSafetyCode)
        {
            return Validation<GeneratedProfileRevisionDto>(normalizedSettingsSafetyCode);
        }

        CalibrationProfileSafetyResult profileValidation =
            CalibrationProfileSafetyValidator.Validate(request.ExactProfileJson, "exactProfileJson");
        if (!profileValidation.IsSafe)
        {
            return Validation<GeneratedProfileRevisionDto>(profileValidation.Code!);
        }
        ...
        if (!await _dbContext.CalibrationAttempts.AnyAsync(
            attempt => attempt.Id == request.SourceAttemptId && attempt.ProjectId == projectId,
            cancellationToken))
        {
            return Validation<GeneratedProfileRevisionDto>("generated_profile_source_attempt_not_found");
        }

        if (request.ParentRevisionId.HasValue &&
            !await _dbContext.GeneratedProfileRevisions.AnyAsync(
                revision => revision.Id == request.ParentRevisionId && revision.ProjectId == projectId,
                cancellationToken))
        {
        ...
        string exactJson = profileValidation.Json!.Value.GetRawText();
        for (int appendAttempt = 0; appendAttempt < MaximumAppendAttempts; appendAttempt++)
        {
            long revisionNumber = (await _dbContext.GeneratedProfileRevisions
                .Where(revision => revision.ProjectId == projectId)
                .Select(revision => revision.RevisionNumber)
                .ToListAsync(cancellationToken))
                .DefaultIfEmpty()
                .Max() + 1;
            GeneratedProfileRevision revision = new()
            {
                Id = Guid.NewGuid(),
                ProjectId = projectId,
                ...
                ProfileType = request.ProfileType.Trim(),
                ...
                SlicerVersion = NormalizeOptional(request.SlicerVersion),
                SlicerContainerDigest = NormalizeOptional(request.SlicerContainerDigest),
                Name = request.Name.Trim(),
                NormalizedSettingsJson = Json(request.NormalizedSettings),
                ExactProfileJson = exactJson,
                Sha256 = CalibrationSnapshotBuilder.ComputeSha256(profileValidation.Json.Value),
                SourceProfileFingerprint = request.SourceProfileFingerprint.Trim(),
                GeneratorVersion = request.GeneratorVersion.Trim(),
                GenerationRequestId = request.GenerationRequestId.Trim(),
                FlowRatio = request.FlowRatio,
                PressureAdvance = request.PressureAdvance,
                PressureAdvanceSmoothTime = request.PressureAdvanceSmoothTime,
                RetractionLength = request.RetractionLength,
                RetractionSpeed = request.RetractionSpeed,
                RetractionMinimumTravel = request.RetractionMinimumTravel,
                RetractionLiftZ = request.RetractionLiftZ,
                NozzleTemperature = request.NozzleTemperature,
                BedTemperature = request.BedTemperature,
                MaximumVolumetricFlow = request.MaximumVolumetricFlow,
                SourceMachineProfileId = request.SourceMachineProfileId,
                SourceProcessProfileId = request.SourceProcessProfileId,
                SourceFilamentProfileId = request.SourceFilamentProfileId,
                CreatedAtUtc = UtcNow(),
                CreatedBySubject = actor.Subject,
            };
            GeneratedProfileRevisionDto result = MapGeneratedProfile(revision);
            _ = _dbContext.GeneratedProfileRevisions.Add(revision);
            AddChange(
                project,
                "generatedProfile",
                revision.Id,
            ...
            {
                _ = await SaveJournaledChangesAsync(cancellationToken);
                return CalibrationApiResult<GeneratedProfileRevisionDto>.Success(
                    result,
                    StatusCodes.Status201Created);
            }
            catch (Exception exception) when (exception is DbUpdateException or DbException)
            {
                ClearTrackedState();
                replay = await FindReplayAsync<GeneratedProfileRevisionDto>(
                    actor,
                    request.ClientId,
                    request.GenerationRequestId,
                    requestHash,
        ...
        throw new InvalidOperationException("The generated-profile revision append retry budget was exhausted.");
    }

    /// <inheritdoc />
    public async Task<CalibrationApiResult<GeneratedProfileRevisionDto>> RecordGeneratedProfileOperationAsync(
        Guid revisionId,
        GeneratedProfileRevisionOperationRequest request,
        string operationType,
        CalibrationActor actor,
    ...
    {
        ArgumentNullException.ThrowIfNull(request);
        if (operationType is not ("export" or "publish") ||
            string.IsNullOrWhiteSpace(request.ClientId) ||
            string.IsNullOrWhiteSpace(request.OperationId) ||
            (operationType == "export" && string.IsNullOrWhiteSpace(request.ExportFormat)))
        {
            return Validation<GeneratedProfileRevisionDto>("generated_profile_operation_invalid");
        }
        ...
        GeneratedProfileRevisionOperation operation = new()
        {
            Id = Guid.NewGuid(),
            GeneratedProfileRevisionId = revision.Id,
            OperationType = operationType,
            OperationId = request.OperationId.Trim(),
            PublishedProfileId = request.PublishedProfileId,
            ExportFormat = NormalizeOptional(request.ExportFormat),
            OccurredAtUtc = UtcNow(),
            ActorSubject = actor.Subject,
        };
        GeneratedProfileRevisionDto result = MapGeneratedProfile(revision);
        _ = _dbContext.GeneratedProfileRevisionOperations.Add(operation);
        CalibrationProject project = await _dbContext.CalibrationProjects.SingleAsync(
            candidate => candidate.Id == revision.ProjectId,
            cancellationToken);
        AddChange(
            project,
        ...
        {
            return await ResolveCreateRaceAsync<GeneratedProfileRevisionDto>(
                actor,
                request.ClientId,
                request.OperationId,
                requestHash,
    ...
    private async Task<GeneratedProfileRevision?> FindVisibleGeneratedProfileAsync(
        Guid revisionId,
        CalibrationActor actor,
        CancellationToken cancellationToken)
    {
    ...
    private static GeneratedProfileRevisionDto MapGeneratedProfile(GeneratedProfileRevision revision) =>
        new(
            revision.Id,
            revision.ProjectId,
            revision.SourceAttemptId,
            revision.ParentRevisionId,
            revision.RevisionNumber,
            revision.ProfileType,
            revision.SchemaVersion,
            revision.SlicerEngine,
            revision.SlicerDistribution,
            revision.SlicerVersion,
            revision.SlicerContainerDigest,
            revision.Name,
            Parse(revision.NormalizedSettingsJson),
            Parse(revision.ExactProfileJson),
            revision.Sha256,
            revision.SourceProfileFingerprint,
