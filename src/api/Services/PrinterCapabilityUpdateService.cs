    ...
    IPrintFarmerTelemetryService telemetry) : BackgroundService
{
    ...
    _logger.LogInformation("{0}", "PrinterCapabilityUpdateService started");
    ...
    _logger.LogInformation("{0}", "PrinterCapabilityUpdateService beginning update cycle");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                _logger.LogDebug("Completed printer capability update cycle, waiting {0} minutes", _updateInterval.TotalMinutes);
    ...
    _logger.LogInformation("{0}", "PrinterCapabilityUpdateService stopped");
    }
    ...
    private async Task UpdateCapabilitiesAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _logger.LogDebug("Found {0} registered printers", printerCount);
            ...
            _logger.LogDebug("Found {0} stale printer capabilities to update", capabilities.Count);
            ...
            _logger.LogInformation("Updating capabilities for {0} printers", capabilities.Count);
            ...
            foreach (PrinterCapabilities? capability in capabilities)
            {
                ...
                try
                {
                    ...
                    _logger.LogDebug("Updated capabilities for printer {0}", capability.Printer.Name);
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "Failed to update capabilities for printer {0}", capability.Printer.Name);
            ...
            _logger.LogInformation("Successfully updated capabilities for {0} printers", capabilities.Count);
        }
        catch (Exception ex)
        {
