...
public class ModelAnalysisService : IModelAnalysisService
{
    ...
    private static async Task<ModelAnalysisResult?> AnalyzeStlAsync(string filePath, CancellationToken cancellationToken)
    {
        ...
        var bytesRead = await fs.ReadAsync(header.AsMemory(0, header.Length), cancellationToken);
        ...
        var countRead = await fs.ReadAsync(countBytes.AsMemory(0, 4), cancellationToken);
        if (countRead < 4)
        {
            return null;
        }
        ...
        var headerString = Encoding.ASCII.GetString(header).Trim().ToLowerInvariant();
            ...
            while ((line = await sr.ReadLineAsync()) != null)
            {
                ...
                if (trimmed.StartsWith("vertex", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (parts.Length >= 4 &&
                        ...
                        double.TryParse(parts[3], NumberStyles.Float, CultureInfo.InvariantCulture, out var vz))
                    {
                        vertexCount++;
                        if (vx < minX)
                        { minX = vx; }
        ...
        else
        {
            ...
            try
            {
                ...
                for (uint i = 0; i < triangleCount; i++)
                {
                    ...
                    for (int v = 0; v < 3; v++)
                    {
                        var baseIndex = 12 + v * 12; // normal(12) + v*(3*4)
                        var vx = BitConverter.ToSingle(buffer, baseIndex);
                        var vy = BitConverter.ToSingle(buffer, baseIndex + 4);
                        var vz = BitConverter.ToSingle(buffer, baseIndex + 8);
                        if (vx < minX)
                        { minX = vx; }
                        if (vy < minY)
                        { minY = vy; }
