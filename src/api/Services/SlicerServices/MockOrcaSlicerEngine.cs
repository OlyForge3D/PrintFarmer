...
public class MockOrcaSlicerEngine : ISlicerEngine
{
    ...
    private static string GenerateMockGcode(DistributedSlicingJob job)
    {
        ...
        gcode.AppendLine($"; Nozzle temp: {job.Profile?.NozzleTemperature ?? 210}°C");
        gcode.AppendLine($"; Bed temp: {job.Profile?.BedTemperature ?? 60}°C");
        gcode.AppendLine($"; Material: {job.Profile?.Material ?? "PLA"}");
        gcode.AppendLine();
