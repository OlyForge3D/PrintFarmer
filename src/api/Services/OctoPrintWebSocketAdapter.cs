...
public sealed class OctoPrintWebSocketAdapter : IDisposable
{
    ...
    /// </summary>
    private static PrinterStateData ParsePrinterState(string json)
    {
        try
        {
            ...
            var stateObj = root.GetProperty("state");
            var flags = stateObj.GetProperty("flags");
            bool operational = flags.GetProperty("operational").GetBoolean();
            bool printing = flags.GetProperty("printing").GetBoolean();
            bool paused = flags.GetProperty("paused").GetBoolean();
            ...
            double? z = null;
            if (root.TryGetProperty("currentZ", out var zProp) && zProp.ValueKind != JsonValueKind.Null)
                z = zProp.GetDouble();
        ...
        {
            throw new InvalidOperationException("Failed to parse OctoPrint printer state", ex);
        }
    }

    /// <summary>
    /// Parses OctoPrint /api/job HTTP response (used for fallback).
    /// </summary>
    private static JobStatusData ParseJobStatus(string json)
    {
        try
        {
            using var doc = JsonDocument.Parse(json);
            ...
            double? progress = null;
            if (root.TryGetProperty("progress", out var progObj) && progObj.ValueKind != JsonValueKind.Null)
            {
                if (progObj.TryGetProperty("completion", out var completion) &&
                    completion.ValueKind != JsonValueKind.Null)
                {
                    progress = completion.GetDouble() * 100.0;
                }
            ...
            string? jobName = null;
            if (root.TryGetProperty("job", out var jobObj) && jobObj.ValueKind != JsonValueKind.Null)
            {
                if (jobObj.TryGetProperty("file", out var fileObj) && fileObj.ValueKind != JsonValueKind.Null)
                {
                    if (fileObj.TryGetProperty("name", out var name) && name.ValueKind != JsonValueKind.Null)
                        jobName = name.GetString();
                }
            }

            return new JobStatusData { Progress = progress, JobName = jobName };
        }
        catch (Exception ex)
        {
            throw new InvalidOperationException("Failed to parse OctoPrint job status", ex);
        }
    }
...
/// <summary>
/// Returned by HTTP fallback for status broadcasts.
/// </summary>
