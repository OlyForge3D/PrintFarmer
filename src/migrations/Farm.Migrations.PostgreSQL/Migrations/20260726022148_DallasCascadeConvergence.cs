...
/// <c>AddPrintedPartsInventory</c> historical migrations. Fresh installs pick up the
/// corrected behavior at their first apply; existing PostgreSQL deployments that already
/// applied the pre-rewrite versions still carry the old <c>SET NULL</c> / <c>CASCADE</c>
/// semantics because EF Core does not diff/replay history to catch onDelete-only changes.
/// Coordinator explicitly required a NEW forward corrective migration in both AppDbContext
/// provider projects rather than relying on an internal runbook. This migration is that
/// migration.
/// </para>
/// <para>
/// <b>Idempotence.</b> Both <c>Up</c> and <c>Down</c> drop-and-recreate each named FK to
/// the target action. On a fresh install where <c>InitialV1</c> already emitted the target
/// action, the drop+recreate is a functional no-op that preserves the same behavior. On an
/// existing PostgreSQL install still carrying the old <c>SetNull</c> or <c>Cascade</c>
/// action, the same drop+recreate converges the FK. Applies safely to both.
/// </para>
/// <para>
...
public partial class DallasCascadeConvergence : Migration
{
    ...
    protected override void Up(MigrationBuilder migrationBuilder)
    {
        ...
        //    Pre-fix action on existing deployments: SetNull against NOT NULL ManufacturerId.
        ...
        //    Pre-fix action on existing deployments: SetNull.
        migrationBuilder.DropForeignKey(
        ...
        //    (matches the InitialV1 rewrite target — Dallas root #1).
        //    Pre-fix action on existing deployments: Cascade.
        migrationBuilder.DropForeignKey(
        ...
        //    (matches the AddPrintedPartsInventory rewrite target — Dallas Fix 4).
        //    Pre-fix action on existing deployments: Cascade.
        migrationBuilder.DropForeignKey(
