...
public partial class HardenCalibrationDispatchFencing : Migration
{
    ...
    protected override void Up(MigrationBuilder migrationBuilder)
    {
        ...
        migrationBuilder.Sql(
            ...
            UPDATE "QueueDispatchOutbox" AS event
            SET "JobRevision" = job."Revision"
            FROM "PrintJobs" AS job
            WHERE job."Id" = event."AggregateId";
            UPDATE "QueueDispatchOutbox" AS event
            SET "DispatchStateRevision" = state."Revision"
            FROM "PrinterDispatchStates" AS state
            WHERE state."PrinterId" = event."PrinterId";
            ...
            SET "QueuePosition" = ranked."NewPosition"
            FROM "RankedJobs" AS ranked
            WHERE ranked."Id" = job."Id";

            INSERT INTO "QueuePositionStates" ("ScopeId", "NextPosition")
            SELECT COALESCE("AssignedPrinterId", '00000000-0000-0000-0000-000000000000'::uuid),
                   MAX("QueuePosition")
            FROM "PrintJobs"
            GROUP BY COALESCE(
                "AssignedPrinterId",
