...
#pragma warning restore IDISP007
    ...
    /// from <see cref="CalibrationRequest.Params"/> (numeric, per method). This keeps the value on
    /// the single existing <c>CalibrationParamsJson</c> column — no schema change — while the
    /// worker's <c>CalibrationParameters.Parse</c> reads each key per its own expected JSON kind.
    /// </summary>
