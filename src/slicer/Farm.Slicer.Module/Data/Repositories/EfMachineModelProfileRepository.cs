...
public class EfMachineModelProfileRepository(SlicerDbContext db) : IMachineModelProfileRepository
{
    ...
    public async Task AddAsync(MachineModelProfile profile, CancellationToken ct = default)
    {
        ...
        {
            // A failed save leaves the entity tracked as Added, so every later save on this context
            // resubmits and fails on it again. Detaching keeps one rejected row from blocking the
            // rows that follow it in a per-row retry loop (#1779).
