...
public static class MoonrakerRestEndpoints
{
    ...
    {
        PrinterAggregate p = Printer(ctx);
        bool connected = p.KlippyState == "ready";
        return MoonrakerJson.WriteResultAsync(ctx, new Dictionary<string, object?>
        {
    ...
    private static Task GetDirectoryAsync(HttpContext ctx)
    {
        ...
        return MoonrakerJson.WriteResultAsync(ctx, BuildDirectoryDto(path, dirs, files));
    }

    private static Dictionary<string, object?> BuildDirectoryDto(string path, IReadOnlyList<string> dirs, IReadOnlyList<VirtualFile> files) => new()
    {
        ["path"] = path,
        ["dirname"] = path,
        ...
        ["files"] = files.Select(FileInfoDto).ToArray(),
    ...
    [
        ...
        0x16, 0x16, 0x16, 0x16, 0x16, 0x16, 0x16, 0x16, 0x16, 0x16, 0x16, 0x16, 0x16, 0x16, 0x16, 0x16,
        0x16, 0x16, 0x16, 0x16, 0x16, 0xFF, 0xC0, 0x00, 0x11, 0x08, 0x00, 0x01, 0x00, 0x01, 0x03, 0x01,
        ...
        0x11, 0x03, 0x11, 0x00, 0x3F, 0x00, 0xFC, 0xFF, 0x00, 0xA2, 0x8A, 0x28, 0x03, 0xFF, 0xD9,
    ];
    ...
    private static Task GetWebcamsListAsync(HttpContext ctx)
    {
        ...
        return MoonrakerJson.WriteBareAsync(ctx, new Dictionary<string, object?>
        {
    ...
    private static Task GetHistoryListAsync(HttpContext ctx)
    {
        ...
        string order = ctx.Request.Query["order"].ToString() is { Length: > 0 } o ? o : "asc";
