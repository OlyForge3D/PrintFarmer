...
- **Verdict evidence:** after opening the PR, a repository maintainer other than the PR author records the exact reviewed head SHA and consensus verdict through `.github/workflows/squad-review-verdict.yml`. Ralph verifies the resulting `squad/pre-pr-verdict` status with `node scripts/ci/verify-squad-verdict.mjs --repo OlyForge3D/PrintFarmer --pr <number> --json`; author comments and lookalike statuses do not count.
- **Supersession and fallback:** any rebase, force-push, or other head change supersedes both approvals and rejections. Until the recorder workflow is on the default branch and a non-author maintainer can use it, author-opened squad PRs require a human GitHub approval before merge.
