...
- **Verdict evidence:** after opening the PR, each reviewer records their verdict as a PR comment (`Squad-Reviewer:` / `Squad-Verdict:` / `Squad-Head-SHA:` — canonical format and rules in `.github/copilot-instructions.md` § "Repository verdict evidence"). `.github/workflows/squad-review-verdict.yml` re-evaluates on every comment, review, and push and publishes the `squad/pre-pr-verdict` commit status. Ralph verifies it with `node scripts/ci/verify-squad-verdict.mjs --repo OlyForge3D/PrintFarmer --pr <number> --json`; pass `--expected-head <recorded-sha>` to audit old-head evidence after a move. Statuses from other workflows and bare PR comments do not count.
- **Supersession and override:** any rebase, force-push, or other head change supersedes both approvals and rejections. A repository administrator overrides the gate unconditionally — either a native GitHub approval at the current head, or a verdict comment naming their own GitHub login as `Squad-Reviewer`.
