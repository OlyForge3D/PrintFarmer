...
- **Gate scope — `squad`-labelled PRs only:** the `squad/pre-pr-verdict` status is evaluated only for PRs carrying the bare `squad` label (a `squad:{member}` assignment label does not count). Squad agents must open PRs with `gh pr create --label squad`; `.github/workflows/squad-pr-label.yml` also auto-labels when the author resolves to a squad identity, but that fails on ~1/3 of PRs so don't rely on it. Unlabelled PRs report `NOT_APPLICABLE` (green, non-blocking) and are **not eligible for Ralph's unattended merge** — a human merges them. That coupling is load-bearing: no label means no gate *and* no autonomy, so a forgotten label costs a manual merge rather than producing an unreviewed auto-merge. Never widen Ralph's merge scope past the `squad` label without widening the gate first.
...
- **Supersession and override:** any rebase, force-push, or other head change supersedes both review records and rejections. A repository administrator overrides the gate unconditionally — either a native GitHub approval at the current head, or a record naming their own GitHub login as `Squad-Reviewer`.
