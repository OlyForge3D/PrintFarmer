...
main() {
  ...
  if (( has_slicer )); then
    ...
    # ISliceJobRepository), and Farm.Modules.Calibration (issue #2038) also
    # references Farm.Slicer.Module directly (slicer-host calibration
    # profile resolution), so a slicer change must re-run all three too.
    test_names+=("Farm.Web.Api.Tests" "Farm.Slicer.Module.Tests" "Farm.OrcaSlicer.Worker.Tests" "Farm.Web.IntegrationTests" "Farm.Infrastructure.Tests" "Farm.Modules.PrintQueue.Tests" "Farm.Modules.Calibration.Tests")
    net_test_bucket_hit=1
  fi
  if (( has_orca )); then
