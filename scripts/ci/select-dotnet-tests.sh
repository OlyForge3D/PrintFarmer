...
main() {
  ...
  local has_tests_integration=0 has_tests_modules=0 has_tests_smartplug=0 has_tests_other=0
  ...
  if (( has_api || has_infra || has_backend || has_backend_core || has_slicer ||
        has_orca || has_smartplug || has_printqueue ||
        has_mig_app || has_mig_slcr ||
        has_tests_api || has_tests_slicer || has_tests_orca ||
        has_tests_integration || has_tests_smartplug || has_tests_printqueue || has_tools )); then
  ...
  if (( has_infra )); then
    ...
    # Farm.Modules.SmartPlug (issue #2036) and Farm.Modules.PrintQueue (issue
    # #2040) also reference Farm.Infrastructure directly, so an infra change
    # must re-run both their test projects too.
    test_names+=("Farm.OrcaSlicer.Worker.Tests" "Farm.Modules.SmartPlug.Tests" "Farm.Modules.PrintQueue.Tests")
    net_test_bucket_hit=1
  fi
  if (( has_backend )); then
  ...
  if (( has_slicer )); then
    ...
    # consumes IArtifactsService/ISliceJobRepository), so a slicer change must
    # re-run it too.
    test_names+=("Farm.Web.Api.Tests" "Farm.Slicer.Module.Tests" "Farm.OrcaSlicer.Worker.Tests" "Farm.Web.IntegrationTests" "Farm.Modules.PrintQueue.Tests")
    net_test_bucket_hit=1
  fi
  if (( has_orca )); then
