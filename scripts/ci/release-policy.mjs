...
export const releaseRequiredChecks = [...releaseBuildChecks, releaseReviewStatus];
...
export function admit(context, selectedHead, versionText, lastStable) {
  ...
  requireThat(context.workflowBranch === 'development', 'Workflow definition must remain on development');
...
export function allocationKey(admission) {
  return hash([admission.repository, admission.workflowIdentity, admission.buildId,
    admission.channel, admission.stage ?? '', admission.sourceCommit, admission.baseVersion]);
}
...
export function validateAdmission(admission) {
  ...
  requireThat(admission.sourceBranch === branch &&
    admission.workflowIdentity === `${repository}/${workflow}@refs/heads/development`,
...
export function validateRecord(record, projected = false) {
  ...
  if (projected) {
    ...
    if (record.protection.dispatchAuthorization) {
      const binding = record.protection.dispatchAuthorization;
      requireThat(binding.sourceCommit === record.sourceCommit &&
        binding.workflowCommit === record.workflowCommit && binding.runId === record.buildId &&
        binding.executionAttempt === record.buildAttempt,
      'Owner dispatch authorization belongs to another release transaction');
    }
...
export function validatePublicAuthorization(identity) {
  ...
  requireThat(identity.sourceBranch === branch &&
    identity.workflowIdentity === `${repository}/${workflow}@refs/heads/development`,
...
export function validateDispatchAssessment(value) {
  ...
  requireString(value.transactionSha256, hashPattern, 'dispatch transaction hash');
  for (const field of ['workflowCommit', 'sourceCommit']) {
    requireString(value[field], shaPattern, `dispatch ${field}`);
  }
...
export function verifyReleaseProtectionRules(evidence, channel, publisherAppId, approvalMode) {
  ...
  requireThat(Array.isArray(rules) && rules.length > 0 && rules.length < 100 &&
    ...
    Array.isArray(branchRulesets) && Array.isArray(rulesets) && rulesets.length === 4,
    'Incomplete protection evidence');
  const branchRuleIds = [...new Set(rules.map(rule => rule.ruleset_id))];
  ...
  for (const rule of branchRulesets) {
    ...
    requireThat(rule.enforcement === 'active' && rule.target === 'branch' &&
      ...
      Array.isArray(rule.conditions.ref_name.exclude) && rule.conditions.ref_name.exclude.length === 0 &&
  ...
  for (const { parameters: policy } of rules.filter(rule => rule.type === 'required_status_checks')) {
    requireThat(policy?.strict_required_status_checks_policy === true &&
      Array.isArray(policy.required_status_checks) && policy.required_status_checks.length > 0 &&
      policy.required_status_checks.every(check => check && typeof check.context === 'string' &&
        check.context.trim() === check.context && check.context.length > 0 && !/[\r\n]/.test(check.context) &&
        (check.integration_id == null || Number.isSafeInteger(check.integration_id) && check.integration_id > 0)),
    'Owner blocker: malformed or non-strict exact-SHA required checks');
    for (const check of policy.required_status_checks) requiredContexts.add(check.context);
  }
...
const protectionProfile = 'printfarmer-release-protection/v4';
...
function approvalAssurance(mode, dispatch = false) {
  if (dispatch) return 'owner-dispatched/self-attested';
...
export function verifyProtectionEvidence(evidence, channel) {
  ...
  if (dispatch) {
    validateDispatchAssessment(evidence.dispatchAuthorization);
    requireThat(evidence.dispatchAuthorization.ownerDispatchEligible &&
      evidence.dispatchAuthorization.channel === channel &&
      evidence.dispatchAuthorization.approvalMode === evidence.approvalMode,
    'Invalid owner dispatch protection authorization');
  }
  requireThat(evidence.approvalAssurance === approvalAssurance(evidence.approvalMode, dispatch),
    'Invalid normalized approval assurance');
  requireThat(evidence.claims && Object.keys(evidence.claims).sort().join() ===
    [...protectionClaims, 'codeOwnerApprovalRequired', 'nonSelfApprovalRequired'].sort().join() &&
    protectionClaims.every(claim => evidence.claims[claim] === (claim === 'manualApprovalRequired' ? !dispatch : true)) &&
    evidence.claims.codeOwnerApprovalRequired === (evidence.approvalMode === 'separation-of-duties') &&
  ...
  const canonical = {
    ...
    verifiedAt: evidence.verifiedAt, policyProfile: evidence.policyProfile,
    ...
    claims: { ...Object.fromEntries(protectionClaims.map(claim => [claim, evidence.claims[claim]])),
      codeOwnerApprovalRequired: evidence.claims.codeOwnerApprovalRequired,
      nonSelfApprovalRequired: evidence.claims.nonSelfApprovalRequired },
    ...(dispatch ? { dispatchAuthorization: evidence.dispatchAuthorization } : {}),
  };
...
export function validateReservationAdmission(state, admission) {
  ...
  if (existing) {
    ...
    requireThat(hash(retryIdentity(existing.admission)) === hash(retryIdentity(admission)),
      'Same allocation key changed its admission');
    return existing;
  }
...
export function verifyConsumer(record, reservation, context, identitySha256 = reservation?.identitySha256 ?? hash(reservation?.record)) {
  ...
  requireThat(record.schema === 1 && record.repository === repository &&
    ...
    record.workflowIdentity === `${repository}/${workflow}@refs/heads/development`,
