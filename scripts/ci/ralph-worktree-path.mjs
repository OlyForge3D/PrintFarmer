...
const defaultFs = { lstat, realpath };

// macOS and Windows hosts use case-insensitive volumes by default.
export const foldPath = (value, platform = process.platform) =>
  (['darwin', 'win32'].includes(platform) ? value.normalize('NFC').toLowerCase() : value.normalize('NFC'));

export const samePath = (left, right, platform) =>
  typeof left === 'string' && typeof right === 'string' && foldPath(left, platform) === foldPath(right, platform);

export function strictlyWithin(root, target, platform) {
  const relative = path.relative(foldPath(root, platform), foldPath(target, platform));
  return Boolean(relative) && relative !== '..' && !relative.startsWith(`..${path.sep}`) && !path.isAbsolute(relative);
}

const overlaps = (left, right, platform) =>
  samePath(left, right, platform) || strictlyWithin(left, right, platform) || strictlyWithin(right, left, platform);

export async function canonicalPath(target, { fs = defaultFs } = {}) {
...
export async function resolveWorktreePath(root, target, {
  exclude = [], fs = defaultFs, platform = process.platform,
} = {}) {
  ...
  if (!strictlyWithin(canonicalRoot, canonical, platform)) fail('Canonical worktree path escapes the configured Ralph worktree root.');
  for (const excluded of exclude.filter((entry) => typeof entry === 'string' && path.isAbsolute(entry))) {
    ...
    if (overlaps(other, canonical, platform)) fail('Worktree path aliases the main checkout or another Ralph checkout.');
  }
  for (let current = canonical; strictlyWithin(canonicalRoot, current, platform); current = path.dirname(current)) {
