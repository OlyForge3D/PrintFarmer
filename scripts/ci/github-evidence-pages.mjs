...
export function evidenceCollection(endpoint) {
  ...
  if (/^actions\/runs\/[1-9][0-9]*\/attempts\/1\/jobs\?per_page=100$/.test(endpoint)) return 'jobs';
...
function pageLinks(link, endpoint, page) {
  ...
  for (const part of link.split(',')) {
    ...
    requireThat(url.origin === expected.origin && url.username === '' && url.password === '' &&
      url.hash === '' && url.pathname === expected.pathname &&
    ...
    requireThat((match[2] !== 'next' || target === page + 1) &&
      ...
      (match[2] !== 'first' || target === 1) &&
      (match[2] !== 'last' || target >= page), 'Skipped or repeated evidence page');
...
export async function readEvidencePages(endpoint, request) {
  ...
  for (let page = 1; page <= maximumPages; page++) {
    ...
    if (complete) {
      requireThat(last === undefined || last === page ||
        (!field && items.length === 0 && last === page - 1), 'Missing terminal evidence page');
  ...
  throw new Error('Evidence page budget exceeded');
}
