...
test('validateLicenseMetadata accepts MSBuild metadata derived from VERSION', async () => {
  const root = await mkdtemp(path.join(tmpdir(), 'printfarmer-version-metadata-'));
  try {
    await writeFile(path.join(root, 'LICENSE'), 'canonical\n');
    await writeFile(path.join(root, 'VERSION'), 'v0.2.3\n');
    await writeFile(path.join(root, 'Directory.Build.props'), [
      '<Project>',
      '  <PropertyGroup>',
    ...
    assert.ok(!hasCode(errors, 'VERSION_METADATA_MISMATCH'));
  } finally {
    await rm(root, { force: true, recursive: true });
  }
});
