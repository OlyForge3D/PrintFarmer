...
const excludedCalibrationExtensions = new Set([
  '.3mf',
  '.csv',
  '.gif',
  '.glb',
...
function validateReview(review, contextPath) {
  const errors = [];
  if (!review || typeof review !== 'object') {
    return [createError('PROVENANCE_REVIEW', contextPath, 'review is required')];
  }

  if (typeof review.reviewer !== 'string' || review.reviewer.trim().length === 0) {
    errors.push(createError('PROVENANCE_REVIEWER', contextPath, 'reviewer is required'));
  }

  if (!isIsoDate(review.reviewDate)) {
    errors.push(createError('PROVENANCE_REVIEW_DATE', contextPath, 'reviewDate must be YYYY-MM-DD'));
  }

  if (review.decision !== 'approved') {
    errors.push(createError('PROVENANCE_APPROVAL', contextPath, 'decision must be approved'));
  }
...
function validateSchemaValue(value, schema, rootSchema, contextPath, errors) {
  if (schema.$ref) {
    const referencedSchema = resolveLocalSchemaReference(rootSchema, schema.$ref);
    if (!referencedSchema) {
      errors.push(createError('PROVENANCE_SCHEMA', contextPath, `unresolved schema reference ${schema.$ref}`));
      return;
    }

    validateSchemaValue(value, referencedSchema, rootSchema, contextPath, errors);
    return;
  }

  if (Object.hasOwn(schema, 'const') && value !== schema.const) {
    errors.push(createError('PROVENANCE_SCHEMA', contextPath, `value must equal ${JSON.stringify(schema.const)}`));
  }

  if (schema.enum && !schema.enum.includes(value)) {
    errors.push(createError('PROVENANCE_SCHEMA', contextPath, `value must be one of ${schema.enum.join(', ')}`));
  }

  if (schema.type === 'object') {
    if (!value || typeof value !== 'object' || Array.isArray(value)) {
      errors.push(createError('PROVENANCE_SCHEMA', contextPath, 'value must be an object'));
      return;
    }

    for (const requiredProperty of schema.required ?? []) {
      if (!Object.hasOwn(value, requiredProperty)) {
        errors.push(createError('PROVENANCE_SCHEMA', `${contextPath}.${requiredProperty}`, 'required property is missing'));
      }
    }
    ...
    if (schema.additionalProperties === false) {
      ...
      for (const property of Object.keys(value)) {
        if (!allowedProperties.has(property)) {
          errors.push(createError('PROVENANCE_SCHEMA', `${contextPath}.${property}`, 'additional property is not allowed'));
        }
      }
    }
    ...
    for (const [property, propertySchema] of Object.entries(schema.properties ?? {})) {
      if (Object.hasOwn(value, property)) {
        validateSchemaValue(value[property], propertySchema, rootSchema, `${contextPath}.${property}`, errors);
      }
    }
    return;
  }
  ...
  if (schema.type === 'array') {
    if (!Array.isArray(value)) {
      errors.push(createError('PROVENANCE_SCHEMA', contextPath, 'value must be an array'));
      return;
    }

    if (schema.minItems !== undefined && value.length < schema.minItems) {
      errors.push(createError('PROVENANCE_SCHEMA', contextPath, `array requires at least ${schema.minItems} items`));
    }

    if (schema.uniqueItems) {
      const serializedItems = value.map((item) => JSON.stringify(item));
      if (new Set(serializedItems).size !== serializedItems.length) {
        errors.push(createError('PROVENANCE_SCHEMA', contextPath, 'array items must be unique'));
      }
    }

    if (schema.items) {
      value.forEach((item, index) =>
        validateSchemaValue(item, schema.items, rootSchema, `${contextPath}[${index}]`, errors));
    }
    return;
  }
  ...
  if (schema.type === 'string') {
    if (typeof value !== 'string') {
      errors.push(createError('PROVENANCE_SCHEMA', contextPath, 'value must be a string'));
      return;
    }

    if (schema.minLength !== undefined && value.length < schema.minLength) {
      errors.push(createError('PROVENANCE_SCHEMA', contextPath, `string requires at least ${schema.minLength} characters`));
    }

    if (schema.pattern && !new RegExp(schema.pattern).test(value)) {
      errors.push(createError('PROVENANCE_SCHEMA', contextPath, `value does not match ${schema.pattern}`));
    }

    if (schema.format === 'date' && !isIsoDate(value)) {
      errors.push(createError('PROVENANCE_SCHEMA', contextPath, 'value must be an ISO date'));
    }

    if (schema.format === 'uri' && !URL.canParse(value)) {
      errors.push(createError('PROVENANCE_SCHEMA', contextPath, 'value must be an absolute URI'));
    }
  }
}

function validateManifestSchema(manifest, schema, manifestPath) {
  const errors = [];
  validateSchemaValue(manifest, schema, schema, manifestPath, errors);
  return errors;
}

function isExcludedCalibrationPath(value) {
  const normalized = normalizeRelativePath(value ?? '');
  return excludedCalibrationExtensions.has(path.extname(normalized).toLowerCase())
    || /(static[-_/ ]?printer|printer[-_/ ]?catalog|data[-_/ ]?table|fixtures?|screenshots?|icons?|models?)([-_/ .]|$)/i
      .test(normalized);
}

function validateApprovedSource(source, contextPath, allowedSourceLicenseExpressions) {
  const errors = [];
  if (!/^[a-z0-9][a-z0-9.-]+$/.test(source.id ?? '')) {
    errors.push(createError('PROVENANCE_SOURCE_ID', contextPath, 'source id must be lowercase and stable'));
  }

  if (!/^https:\/\/github\.com\/[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(source.repositoryUrl ?? '')) {
    errors.push(createError('PROVENANCE_SOURCE_URL', contextPath, 'repositoryUrl must be a canonical HTTPS GitHub repository URL'));
  }

  if (!/^[0-9a-f]{40}$/.test(source.commitSha ?? '')) {
    errors.push(createError('PROVENANCE_SOURCE_COMMIT', contextPath, 'commitSha must be a full lowercase 40-character SHA'));
  }

  if (!source.tag || /^(main|master|head|latest|development|release)$/i.test(source.tag) || source.tag.includes('refs/heads/')) {
    errors.push(createError('PROVENANCE_SOURCE_TAG', contextPath, 'tag must be immutable and cannot name a branch or latest release'));
  }

  if (!allowedSourceLicenseExpressions.includes(source.licenseExpression)) {
    errors.push(createError(
      'PROVENANCE_SOURCE_LICENSE',
      contextPath,
      `source license must be one of: ${allowedSourceLicenseExpressions.join(', ')}`,
    ));
  }

  if (!source.licenseEvidence?.path || !/^(sha1:[0-9a-f]{40}|sha256:[0-9a-f]{64})$/.test(source.licenseEvidence?.blobDigest ?? '')) {
    errors.push(createError('PROVENANCE_LICENSE_EVIDENCE', contextPath, 'license evidence requires a path and immutable blob digest'));
  }

  errors.push(...validateReview(source.review, contextPath));
  return errors;
}
...
export async function validateProvenanceManifest(repoRoot, manifest) {
  const errors = [];
  const manifestPath = 'compliance/calibration-provenance.json';
  const schemaPath = path.join(repoRoot, 'compliance', 'calibration-provenance.schema.json');
  if (!(await pathExists(schemaPath))) {
    errors.push(createError('PROVENANCE_SCHEMA', 'compliance/calibration-provenance.schema.json', 'schema is missing'));
  } else {
    errors.push(...validateManifestSchema(manifest, await readJson(schemaPath), manifestPath));
  }

  if (manifest.schemaVersion !== 1) {
    errors.push(createError('PROVENANCE_SCHEMA', manifestPath, 'schemaVersion must be 1'));
  }

  if (!/^\d+\.\d+\.\d+$/.test(manifest.policyVersion ?? '')) {
    errors.push(createError('PROVENANCE_POLICY_VERSION', manifestPath, 'policyVersion must be semantic version text'));
  }

  if (!isIsoDate(manifest.lastReviewed)) {
    errors.push(createError('PROVENANCE_REVIEW_DATE', manifestPath, 'lastReviewed must be YYYY-MM-DD'));
  }

  let hasInvalidArrayShape = false;
  for (const property of [
    'allowedSourceLicenseExpressions',
    'governedPathPatterns',
    'approvedSources',
    'entries',
    'referenceRecords',
    'exceptions',
    ...
    if (!Array.isArray(manifest[property])) {
      errors.push(createError('PROVENANCE_SCHEMA', manifestPath, `${property} must be an array`));
      hasInvalidArrayShape = true;
    }
  }

  if (hasInvalidArrayShape) {
    return errors;
  }

  if (!isSorted(manifest.approvedSources.map((source) => source.id))) {
    errors.push(createError('PROVENANCE_ORDER', manifestPath, 'approvedSources must be sorted by id'));
  }

  if (!isSorted(manifest.entries.map((entry) => entry.id))) {
    errors.push(createError('PROVENANCE_ORDER', manifestPath, 'entries must be sorted by id'));
  }

  if (!isSorted(manifest.referenceRecords.map((record) => record.id))) {
    errors.push(createError('PROVENANCE_ORDER', manifestPath, 'referenceRecords must be sorted by id'));
  }

  if (!isSorted(manifest.exceptions.map((exception) => exception.id))) {
    errors.push(createError('PROVENANCE_ORDER', manifestPath, 'exceptions must be sorted by id'));
  }

  const sourceIds = new Set();
  const sourcesById = new Map();
  for (const source of manifest.approvedSources) {
    const contextPath = `${manifestPath}:approvedSources:${source.id ?? '<missing>'}`;
    if (sourceIds.has(source.id)) {
      errors.push(createError('PROVENANCE_DUPLICATE_SOURCE', contextPath, `duplicate source id ${source.id}`));
    }

    sourceIds.add(source.id);
    sourcesById.set(source.id, source);
    errors.push(...validateApprovedSource(
      source,
      contextPath,
      manifest.allowedSourceLicenseExpressions,
    ));
  }
  ...
  for (const entry of manifest.entries) {
    const contextPath = `${manifestPath}:entries:${entry.id ?? '<missing>'}`;
    if (entryIds.has(entry.id)) {
      errors.push(createError('PROVENANCE_DUPLICATE_ENTRY', contextPath, `duplicate entry id ${entry.id}`));
    }

    entryIds.add(entry.id);
    if (!sourceIds.has(entry.sourceId)) {
      errors.push(createError('PROVENANCE_UNAPPROVED_SOURCE', contextPath, `sourceId ${entry.sourceId ?? '<missing>'} is not approved`));
    }

    if (!/^(sha1:[0-9a-f]{40}|sha256:[0-9a-f]{64})$/.test(entry.sourceBlobDigest ?? '')) {
      errors.push(createError('PROVENANCE_BLOB_DIGEST', contextPath, 'sourceBlobDigest must be an immutable SHA-1 or SHA-256 digest'));
    }

    if (!isSafeRepositoryPath(entry.sourcePath ?? '')) {
      errors.push(createError('PROVENANCE_SOURCE_PATH', contextPath, 'sourcePath must be a safe repository-relative path'));
    }

    if (!isSafeRepositoryPath(entry.destinationPath ?? '')) {
      errors.push(createError('PROVENANCE_DESTINATION_PATH', contextPath, 'destinationPath must be a safe repository-relative path'));
      continue;
    }

    const destinationPath = normalizeRelativePath(entry.destinationPath);
    if (destinationPaths.has(destinationPath)) {
      errors.push(createError('PROVENANCE_DUPLICATE_DESTINATION', contextPath, `destination ${destinationPath} appears more than once`));
    }

    destinationPaths.add(destinationPath);
    if (isExcludedCalibrationPath(destinationPath) || isExcludedCalibrationPath(entry.sourcePath)) {
      errors.push(createError(
        'PROVENANCE_EXCLUDED_CONTENT',
        contextPath,
        'source and destination paths must exclude static printer data, fixtures, and unverified assets',
      ));
    }

    const approvedSource = sourcesById.get(entry.sourceId);
    if (approvedSource && entry.upstreamSpdx !== approvedSource.licenseExpression) {
      errors.push(createError(
        'PROVENANCE_LICENSE_MISMATCH',
        contextPath,
        `upstreamSpdx must equal approved source license ${approvedSource.licenseExpression}`,
      ));
    }

    if (!['adapted', 'copied', 'generated'].includes(entry.derivation)) {
      errors.push(createError('PROVENANCE_DERIVATION', contextPath, 'derivation must be adapted, copied, or generated'));
    }

    if (!Array.isArray(entry.preservedNotices) || entry.preservedNotices.length === 0) {
      errors.push(createError('PROVENANCE_NOTICE', contextPath, 'at least one preserved notice is required'));
    }

    if (!entry.modificationSummary || !entry.architecturalChanges || !entry.upstreamCopyright || !entry.upstreamSpdx) {
      errors.push(createError(
        'PROVENANCE_DOCUMENTATION',
        contextPath,
        'copyright, SPDX evidence, modification summary, and architectural changes are required',
      ));
    }

    if (!/^v\d+\.\d+\.\d+([-.+][0-9A-Za-z.-]+)?$/.test(entry.firstPrintFarmerVersion ?? '')) {
      errors.push(createError('PROVENANCE_FIRST_VERSION', contextPath, 'firstPrintFarmerVersion must be a v-prefixed semantic version'));
    }

    errors.push(...validateReview(entry.review, contextPath));

    if (!Array.isArray(entry.tests) || entry.tests.length === 0) {
      errors.push(createError('PROVENANCE_TESTS', contextPath, 'at least one related test path is required'));
    } else {
      for (const testPath of entry.tests) {
        if (!isSafeRepositoryPath(testPath) || !(await pathExists(path.join(repoRoot, testPath)))) {
          errors.push(createError('PROVENANCE_TEST_PATH', contextPath, `test path does not exist: ${testPath}`));
        }
      }
    }

    const absoluteDestination = path.join(repoRoot, destinationPath);
    if (!(await pathExists(absoluteDestination))) {
      errors.push(createError('PROVENANCE_DESTINATION_MISSING', contextPath, `destination does not exist: ${destinationPath}`));
      continue;
    }

    const destinationContent = await readFile(absoluteDestination);
    if (!/^[0-9a-f]{64}$/.test(entry.destinationSha256 ?? '')
      || sha256(destinationContent) !== entry.destinationSha256) {
      errors.push(createError(
        'PROVENANCE_DESTINATION_DIGEST',
        contextPath,
        `destinationSha256 does not match ${destinationPath}`,
      ));
    }

    const destinationText = destinationContent.toString('utf8');
    if (!destinationText.includes(`PrintFarmer-Provenance-ID: ${entry.id}`)) {
      errors.push(createError('PROVENANCE_MARKER', contextPath, `destination must contain PrintFarmer-Provenance-ID: ${entry.id}`));
    }

    if (!destinationText.includes('SPDX-License-Identifier:')) {
      errors.push(createError('PROVENANCE_SPDX', contextPath, 'destination must preserve an SPDX-License-Identifier marker'));
    }

    for (const notice of entry.preservedNotices ?? []) {
      if (!destinationText.includes(notice)) {
        errors.push(createError('PROVENANCE_NOTICE_PRESERVATION', contextPath, `destination does not contain preserved notice: ${notice}`));
      }
    }
  }

  const allSourceFiles = await walkFiles(repoRoot, 'src');
  const governedMatchers = manifest.governedPathPatterns.map(globToRegExp);
  const referencePaths = new Set(manifest.referenceRecords
    .map((record) => normalizeRelativePath(record.destinationPath ?? '')));
  for (const filePath of allSourceFiles) {
    if (governedMatchers.some((matcher) => matcher.test(filePath))
      && !destinationPaths.has(filePath)
      && !referencePaths.has(filePath)) {
      errors.push(createError(
        'PROVENANCE_MANIFEST_ENTRY_MISSING',
        filePath,
        'file is in a governed derived-source path but has no provenance entry',
      ));
    }
    ...
    if (markerMatch && !entryIds.has(markerMatch[1])) {
      errors.push(createError(
        'PROVENANCE_MARKER_UNREGISTERED',
        filePath,
        `provenance marker ${markerMatch[1]} has no manifest entry`,
      ));
    }
  }

  for (const record of manifest.referenceRecords) {
    const contextPath = `${manifestPath}:referenceRecords:${record.id ?? '<missing>'}`;
    if (!isSafeRepositoryPath(record.destinationPath ?? '')
      || !(await pathExists(path.join(repoRoot, record.destinationPath ?? '')))) {
      errors.push(createError('PROVENANCE_REFERENCE_DESTINATION', contextPath, 'reference destination must exist'));
    }

    if (!Array.isArray(record.referenceUrls) || record.referenceUrls.length === 0
      || record.referenceUrls.some((url) => !/^https:\/\//.test(url))) {
      errors.push(createError('PROVENANCE_REFERENCE_URL', contextPath, 'reference records require HTTPS URLs'));
    }

    if (!record.statement || !/independent/i.test(record.statement)) {
      errors.push(createError(
        'PROVENANCE_REFERENCE_STATEMENT',
        contextPath,
        'reference record must explicitly state independent implementation',
      ));
    }

    errors.push(...validateReview(record.review, contextPath));
  }

  for (const exception of manifest.exceptions) {
    errors.push(...validateExceptionRecord(
      exception,
      `${manifestPath}:exceptions:${exception.id ?? '<missing>'}`,
    ));
  }
...
export async function validateLicenseMetadata(repoRoot, licensingPolicy) {
  ...
  if (parsedCurrentVersion && await pathExists(buildPropsPath)) {
    ...
    if (versionPrefix !== packageVersion) {
      errors.push(createError(
        'VERSION_METADATA_MISMATCH',
        'Directory.Build.props',
        `VersionPrefix must match VERSION (${packageVersion})`,
      ));
    }
  }
...
export async function validateRepository(repoRoot, options = {}) {
  ...
  const provenancePath = path.join(repoRoot, 'compliance', 'calibration-provenance.json');
  const errors = [];

  for (const requiredPolicyPath of [licensingPolicyPath, dependencyPolicyPath, provenancePath]) {
