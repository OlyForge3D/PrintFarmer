...
struct OversightRowSubtitleContext: Equatable {
    ...
    var navigationPreference: NavigationLayoutPreference?
    ...
    var automaticDerivation: NavigationShellDerivation?
...
enum OversightRowSubtitles {
    ...
    ///
    /// Up to two fragments are joined with ", " so the row does not grow
    /// past a single subtitle line on small screens.
    ...
    /// `"scheduled"` when we have no `daysUntilDue`.
    private static func maintenanceState(
        ...
        if days <= 0 {
            // Belt-and-braces: negative days without isOverdue set.
            return "overdue"
        }
