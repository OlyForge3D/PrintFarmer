...
final class MockPrinterService: PrinterServiceProtocol, @unchecked Sendable {
    ...
    func setActiveSpool(
        ...
        if let hook = beforeSetActiveSpool { await hook() }
    ...
    func bindToolheadSpool(printerId: UUID, toolheadIndex: Int, request: ToolheadSpoolBindRequest, idempotencyKey: String) async throws -> CommandResult {
        if let hook = beforeBindToolheadSpool { await hook() }
    ...
    var afterSetTemperatures: (@Sendable () -> Void)?
    ...
    func setTemperatures(printerId: UUID, hotend: Double?, bed: Double?) async throws {
        ...
        if let error = errorToThrow { throw error }
        afterSetTemperatures?()
    }

    var beforeMotion: (@Sendable () async -> Void)?
    private(set) var motionCallCount = 0

    func home(printerId: UUID, axes: [String]) async throws {
        motionCallCount += 1
        homeCalledWith = (printerId, axes)
        if let beforeMotion { await beforeMotion() }
        if let error = errorToThrow { throw error }
    }
