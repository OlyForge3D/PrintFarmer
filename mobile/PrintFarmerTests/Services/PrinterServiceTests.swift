...
final class PrinterServiceTests: XCTestCase {
    ...
    func testDirectMotionTransportErrorsNeverReplay() async throws {
        for code in [URLError.Code.timedOut, .networkConnectionLost, .cancelled] {
            ...
            do {
                ...
                XCTFail("A transport failure cannot imply command acceptance")
            } catch { }
            XCTAssertEqual(mockAPIClient.capturedRequests.count, 1)
            XCTAssertTrue(mockAPIClient.capturedRequests.first?.url?.path.hasSuffix("/home") == true)
            XCTAssertNil(mockAPIClient.capturedRequests.first?.value(forHTTPHeaderField: "Idempotency-Key"))
        }
    }

    func testAlreadyCancelledDirectMotionDoesNotSend() async {
        ...
        do {
            ...
            XCTFail("Cancelled command must not dispatch")
        } catch { }
