...
STATUS=$?
if [[ $STATUS -ne 0 ]]; then
  echo "❌ match failed while creating new certs. Aborting before touching GitHub secrets."
  exit $STATUS
fi
...
echo "-> Clearing clipboard (it held the passphrase for the paste-into-Passwords step)..."
printf '' | pbcopy
