...
- Command feedback appears beneath the originating **Heat**, **Move & home**,
  ...
  Home, jog and absolute commands settle when their direct HTTP request returns.
  Acceptance explicitly does not confirm physical completion. Pending requests
  retain local single-flight ownership even if observation stops; errors and
  timeouts instruct the operator to inspect the machine before another action.
  No motion journal, receipt, recovery panel or operation-status subscription
  exists. Ordinary telemetry and capability/safety refreshes remain independent,
  and never replay commands.
