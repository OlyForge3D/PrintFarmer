...
Render `/login` with a dedicated page auth surface instead of the shared modal
backdrop.

Keep `LoginModal` and `RegisterModal` reusable for in-app authentication flows,
but allow them to switch between `modal` and `page` presentation.

## Rationale

The shared modal wrapper always paints a darkened overlay. That is correct when
sign-in is layered on top of existing app content, but it is the wrong visual
model for the standalone `/login` route because there is nothing underneath to
dim.

On Light theme in particular, the backdrop turns the intended `bg-pf-bg-0`
canvas into a muted gray field that reads like a disabled screen. A centered
card on the page background preserves theme fidelity while keeping the auth
components reusable.

## Impact

The login route now presents as a clean centered card across themes,
especially Light theme.

The shared modal behavior remains available for layout-triggered auth prompts,
so the fix stays route-specific instead of changing app-wide modal behavior.


---
# Decision: Locations Dashboard Navigation
