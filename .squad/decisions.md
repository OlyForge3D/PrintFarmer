# Recent Decisions (from inbox, merged 2026-09-04T16:56:30Z)

## Dallas — DateFormatter Spike #2364 Analysis Gate

### 2026-09-04T16:39:53-07:00: DateFormatter spike #2364 analysis gate

**By:** Dallas (Lead), requested by Frost via Ralph

**What:** Closed #2364 as *measured / not-warranted*. **No implementation issue filed**; `mobile/PrintFarmer/Extensions/Formatting+Extensions.swift` stays as written. Set the reusable acceptance threshold for this class of iOS frame-time question: fix iff **T1** (formatter-construction symbol union ≥ **1.0 %** of sampled main-thread CPU in a ≥ 20 s window with the row mounted and ≥ 1 row visible) **or T2** (that union appears in ≥ 1 hitch or ≥ 1 microhang > 250 ms). Also decided the two blocking correctness questions, one of which corrects a prior answer. Ended a six-round measurement loop that was blocked on a farm workload no analysis session can create.

**Why:** The decisive evidence is workload-independent, which is why another Instruments run could not have changed the verdict:

1. **Scope collapse (exhaustive call-graph analysis).** Of the four flagged per-call constructions: `etaFormatted` (lines 59, 63) has **zero call sites in the entire iOS target** — two of four are unreachable dead code. `shortTimeFormatted` (line 25) has **one** doubly-conditional call site (`DashboardView.swift:1114`), and its symbol family (`DateFormatter.init` / `__CreateCFDateFormatter`) logged **0 samples across all 192.6 s** of measurement. `relativeFormatted` (line 6) is **9 of 10** call sites and the only construction with any measured cost.
2. **The one construction that matters cannot be hoisted as proposed (verified by compilation).** Under `SWIFT_VERSION = 6.0`, a nonisolated global `RelativeDateTimeFormatter` is inferred `@MainActor` (non-`Sendable`) and is rejected from the nonisolated `extension Date` — hard compile error. It requires `nonisolated(unsafe)`, an unaudited thread-safety assertion on a type Apple does not document as thread-safe, or `@MainActor`, which is source-breaking. Both are **worse on correctness** than the current code. The cited precedents do not transfer: `FilamentCoverageBadge` compiles only because `View` makes it `@MainActor`; `APIClient` covers `ISO8601DateFormatter`, a different type.
3. **Measured baseline is 99× below T1 and clear of T2.** Heaviest exercised case (Notifications, 44.0 s): relative-formatter construction **0.7 ms = 0.0101 %** of 6,902.2 ms main-thread CPU. `DateFormatter` construction: **0 samples** everywhere. **0 hitches / 0 hangs across 192.6 s**.

**Correctness questions — decided:**

- **Swift 6 concurrency (prior 2026-09-03 answer CORRECTED).** A nonisolated global `DateFormatter` **compiles** (it is `Sendable`, matching Apple's documented iOS 7+ thread safety); nonisolated global `RelativeDateTimeFormatter` **does not**.
- **Runtime locale / time-zone (prior answer UPHELD).** Changes must stay observable; app restart is not acceptable. The per-call implementation is correct by construction.

**Artifact:** https://github.com/OlyForge3D/PrintFarmer/issues/2364#issuecomment-5547770003

---

## Parker — Make yamllint fail closed
...
## Ripley — Profile-family Phases 0–1 Frontend Implementation
