...
- 2026-08 — Spaghetti shield canonical status source (#309): `FailureDetectionPrinterStatusDto.isPrinting` is stale (backend polling, ~30s cycle). The canonical live "is printing" in React is `printer.state === 'Printing'` (SignalR-updated). `CompactPrinterCard` uses `state.toLowerCase().includes('printing')` to also catch Pausing. When the two diverge, pass the live value as `isPrinting` prop to `FailureDetectionMonitoringBadge`/`FailureDetectionMonitoringOverlay` and build an `effectiveStatus` that overrides both `isPrinting` and `reason`. Badge is in `src/Web/ReactApp/src/features/printers/components/FailureDetectionMonitoringBadge.tsx`.
- 2026-08 — Spaghetti shield `stalePrintingMismatch` discriminator (#313): the condition that overrides a stale DTO reason must be `status.state === 'idle'` only — never `|| status.state === 'disabled'`. `idle` is the only state that can be stale (polling lag while the job just started). `disabled` is always intentional: either the global feature is off (*"Failure detection is disabled in Settings."*) or the backend does not support it. The DTO default is also `disabled` with an empty reason. Adding `disabled` to the mismatch condition would silently erase the operator-visible reason for unsupported backends. Backend reason strings live in `PrintFailureMonitorService.cs` (const `NotPrintingReason`, `StartingIdleReason`, `WarmupIdleReason`). Regression tests for both cases are in `FailureDetectionMonitoringBadge.test.tsx` and `FailureDetectionMonitoringOverlay.test.tsx`.
