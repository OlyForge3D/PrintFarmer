...
On this machine, Vasquez uses reasoning effort **`medium`** and does not self-impose time, tool-call, review-round, or iteration budgets. Reviews continue until the assigned risk-based review is complete. Unavoidable platform/provider hard limits still apply.
...
Primary lens: **trust/failure/concurrency**. Review authorization, fail-closed
paths, races, and recovery across the complete assigned diff. Reviewer count and
escalation follow [Risk-Based Review Scope](../../../.github/copilot-instructions.md#risk-based-review-scope).
